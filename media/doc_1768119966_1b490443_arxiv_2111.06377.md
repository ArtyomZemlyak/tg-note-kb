...
**File:** doc_1768119966_1b490443_arxiv_2111.06377.pdf
**Original:** arxiv_2111.06377.pdf
**Received:** 1768119966
...
Deep learning has witnessed an explosion of architectures of continuously growing capability and capacity [33, 25, 57]. Aided by the rapid gains in hardware, models today can easily overfit one million images [13] and begin to demand hundreds of millions of-often publicly inaccessiblelabeled images [16].
...
| (a) Decoder depth . A deep decoder can im- prove linear probing accuracy.     | (a) Decoder depth . A deep decoder can im- prove linear probing accuracy.     | (a) Decoder depth . A deep decoder can im- prove linear probing accuracy.     | (a) Decoder depth . A deep decoder can im- prove linear probing accuracy.     | (b) Decoder width . The decoder can be nar- rower than the encoder (1024-d). | (b) Decoder width . The decoder can be nar- rower than the encoder (1024-d). | (b) Decoder width . The decoder can be nar- rower than the encoder (1024-d). | (b) Decoder width . The decoder can be nar- rower than the encoder (1024-d). | (b) Decoder width . The decoder can be nar- rower than the encoder (1024-d).         | (c) Mask token . An encoder without mask to- kens is more accurate and faster (Table 2). | (c) Mask token . An encoder without mask to- kens is more accurate and faster (Table 2). | (c) Mask token . An encoder without mask to- kens is more accurate and faster (Table 2). | (c) Mask token . An encoder without mask to- kens is more accurate and faster (Table 2). | (c) Mask token . An encoder without mask to- kens is more accurate and faster (Table 2). |
...
| (d) Reconstruction target . Pixels as recon- struction targets are effective. | (d) Reconstruction target . Pixels as recon- struction targets are effective. | (d) Reconstruction target . Pixels as recon- struction targets are effective. | (d) Reconstruction target . Pixels as recon- struction targets are effective. | (e) Data augmentation . Our MAEworks with minimal or no augmentation.        | (e) Data augmentation . Our MAEworks with minimal or no augmentation.        | (e) Data augmentation . Our MAEworks with minimal or no augmentation.        | (e) Data augmentation . Our MAEworks with minimal or no augmentation.        | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations. | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations.     | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations.     | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations.     | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations.     | (f) Mask sampling . Random sampling works the best. See Figure 6 for visualizations.     |
...
Wealso study grid-wise sampling, which regularly keeps one of every four patches (Figure 6 right). This is an easier task and has lower training loss. The reconstruction is sharper. However, the representation quality is lower.
...
| dataset   |   ViT-B |   ViT-L |   ViT-H |   ViT-H 448 | prev best   |
|-----------|---------|---------|---------|-------------|-------------|
| iNat 2017 |    70.5 |    75.7 |    79.3 |        83.4 | 75.4 [55]   |
| iNat 2018 |    75.4 |    80.1 |    83   |        86.8 | 81.2 [54]   |
| iNat 2019 |    80.5 |    83.4 |    85.7 |        88.3 | 84.1 [54]   |
| Places205 |    63.9 |    65.8 |    65.9 |        66.8 | 66.0 [19] † |
| Places365 |    57.9 |    59.4 |    59.8 |        60.3 | 58.0 [40] ‡ |

Table 7. Pixels vs . tokens as the MAE reconstruction target. glyph[triangle] is the difference between using dVAE tokens and using normalized pixels. The difference is statistically insignificant.

|                  |       |       |       | COCO   | COCO   | ADE20K   | ADE20K   |
|------------------|-------|-------|-------|--------|--------|----------|----------|
...
| glyph[triangle]  | 0.0   | -0.2  | 0.0   | 0.0    | -0.1   | 0.0      | -0.2     |
...
| weight decay           | 0.05                         |
| optimizer momentum     | β 1 ,β 2 =0 . 9 , 0 . 95 [6] |
| batch size             | 4096                         |
...
| config                                                                                                                                                                                                                        | value                                                                                                                                    |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| optimizer base learning rate weight decay optimizer momentum layer-wise lr decay [10, batch size learning rate schedule warmup epochs training epochs augmentation label smoothing [52] mixup [69] cutmix [68] drop path [30] | AdamW 1e-3 0.05 β 1 ,β 2 =0 . 9 , 0 . 999 0.75 1024 cosine decay 5 100 (B), 50 (L/H) RandAug (9, 0.5) [12] 0.1 0.8 1.0 0.1 (B/L) 0.2 (H) |
...
| config                                                                                                                                                                                                                         | value                                                                                                                                       |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| optimizer base learning rate weight decay optimizer momentum batch size learning rate schedule warmup epochs training epochs augmentation label smoothing [52] mixup [69] cutmix [68] drop path [30] exp. moving average (EMA) | AdamW 1e-4 0.3 β 1 ,β 2 =0 . 9 , 0 . 95 4096 cosine decay 20 300 (B), 200 (L/H) RandAug (9, 0.5) [12] 0.1 0.8 1.0 0.1 (B), 0.2 (L/H) 0.9999 |
...
|----------|---------|----------|--------|
| iGPT [6] | iGPT-L  | 1362M    | 69.0   |
| iGPT [6] | iGPT-XL | 6801M    | 72.0   |
| BEiT [2] | ViT-L   | 304M     | 52.1 † |
| MAE      | ViT-B   | 86M      | 68.0   |
| MAE      | ViT-L   | 304M     | 75.8   |
| MAE      | ViT-H   | 632M     | 76.6   |
...
| our supervised training baselines: | our supervised training baselines: | our supervised training baselines: | our supervised training baselines: | our supervised training baselines: | our supervised training baselines: |
...
Example:
```markdown
![Description based on OCR](../media/doc_1768119966_1b490443_arxiv_2111.06377.pdf) <!-- TODO: Broken image path -->
