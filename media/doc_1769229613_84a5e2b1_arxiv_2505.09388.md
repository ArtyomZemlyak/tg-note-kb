...
**File:** doc_1769229613_84a5e2b1_arxiv_2505.09388.pdf
**Original:** arxiv_2505.09388.pdf
**Received:** 1769229613
...
- (1) Comparedwith the previously open-source SOTA dense and MoE base models (such as DeepSeekV3 Base, Llama-4-Maverick Base, and Qwen2.5-72B-Base), Qwen3-235B-A22B-Base outperforms these models in most tasks with significantly fewer total parameters or activated parameters.
...
|------------------------------------------------|--------------------|---------------------|-------------------------|--------------------|------------------------|
| Architecture # Total Params # Activated Params | Dense 72B 72B      | MoE 271B 37B        | MoE 402B 17B            | MoE 671B 37B       | MoE 235B 22B           |
...
| Multilingual Tasks                             | Multilingual Tasks | Multilingual Tasks  | Multilingual Tasks      | Multilingual Tasks | Multilingual Tasks     |
...
|------------------------------------------------|--------------------|--------------------|--------------------|----------------------|--------------------|
| Architecture # Total Params # Activated Params | Dense 32B 32B      | Dense 72B 72B      | Dense 27B 27B      | MoE 109B 17B         | Dense 32B 32B      |
| General Tasks                                  | General Tasks      | General Tasks      | General Tasks      | General Tasks        | General Tasks      |
...
|------------------------------------------------|--------------------|--------------------|--------------------|----------------------|--------------------|----------------------|
| Architecture # Total Params # Activated Params | Dense 12B 12B      | Dense 14B 14B      | Dense 32B 32B      | MoE 42B 6B           | Dense 14B 14B      | MoE 30B 3B           |
...
| Math &STEM Tasks                               | Math &STEM Tasks   | Math &STEM Tasks   | Math &STEM Tasks   | Math &STEM Tasks     | Math &STEM Tasks   | Math &STEM Tasks     |
...
| Coding Tasks                                   | Coding Tasks       | Coding Tasks       | Coding Tasks       | Coding Tasks         | Coding Tasks       | Coding Tasks         |
...
| Multilingual Tasks                             | Multilingual Tasks | Multilingual Tasks | Multilingual Tasks | Multilingual Tasks   | Multilingual Tasks | Multilingual Tasks   |
...
|------------------------------------------------|--------------------|--------------------|--------------------|--------------------|
| Architecture # Total Params # Activated Params | Dense 8B 8B        | Dense 7B 7B        | Dense 14B 14B      | Dense 8B 8B        |
| General Tasks                                  | General Tasks      | General Tasks      | General Tasks      | General Tasks      |
...
| Math &STEM Tasks                               | Math &STEM Tasks   | Math &STEM Tasks   | Math &STEM Tasks   | Math &STEM Tasks   |
...
| Multilingual Tasks                             | Multilingual Tasks | Multilingual Tasks | Multilingual Tasks | Multilingual Tasks |
...
|-----------------------------|--------------------|--------------------|--------------------|--------------------|
| Architecture # Total Params | Dense 4B           | Dense 3B           | Dense 7B           | Dense 4B           |
| General Tasks               | General Tasks      | General Tasks      | General Tasks      | General Tasks      |
...
|------------------------------------------------|---------------------|--------------------|--------------------|---------------------|--------------------|
| Architecture # Total Params # Activated Params | Dense 0.5B 0.5B     | Dense 0.6B 0.6B    | Dense 1B 1B        | Dense 1.5B 1.5B     | Dense 1.7B 1.7B    |
| General Tasks                                  | General Tasks       | General Tasks      | General Tasks      | General Tasks       | General Tasks      |
...
Table 9: Examples of SFT data for thinking and non-thinking modes during the thinking mode fusion stage. For the thinking mode, the /think flag can be omitted since it represents the default behavior. This feature has been implemented in the chat template 1 supported by the Hugging Face's tokenizer, where the thinking mode can be disabled using an additional parameter enable thinking=False .

| Thinking Mode                                                                                                                   | Non-Thinking Mode                                                                                             |
|---------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|
| <|im start|>user { query } /think<|im end|> <|im start|>assistant <think> { thinking content } </think> { response } <|im end|> | <|im start|>user { query } /no think<|im end|> <|im start|>assistant <think> </think> { response } <|im end|> |
...
Thinking Budget. An additional advantage of Thinking Mode Fusion is that, once the model learns to respond in both non-thinking and thinking modes, it naturally develops the ability to handle intermediate cases-generating responses based on incomplete thinking. This capability lays the foundation for implementing budget control over the model's thinking process. Specifically, when the length of the model's thinking reaches a user-defined threshold, we manually halt the thinking process and insert the stop-thinking instruction: ' Considering the limited time by the user, I have to give the solution based on the thinking directly now. \ n&lt;/think&gt;. \ n \ n '. After this instruction is inserted, the model proceeds to generate a final response based on its accumulated reasoning up to that point. It is worth noting that this ability is not explicitly trained but emerges naturally as a result of applying Thinking Mode Fusion.
...
|--------------------|----------------------------------|---------------------------------|--------------|--------------|-----------------|
|                    | Architecture # Activated Params  | Dense                           | Dense 32B    | Dense 14B    | MoE 3B          |
|                    |                                  | 32B                             |              |              |                 |
|                    | # Total Params                   | 32B                             | 32B          | 14B          | 30B             |
|                    | MMLU-Redux                       | 88.2                            | 90.0         | 88.6         | 89.5            |
| General            |                                  |                                 |              | 64.0         |                 |
|                    | GPQA-Diamond                     | 62.1                            | 65.6         | 86.2         | 65.8 86.6       |
| Tasks              | C-Eval LiveBench 2024-11-25      | 82.2 45.6                       | 88.4 72.0    | 71.3         | 74.3            |
|                    | IFEval strict prompt             | 72.5                            | 83.9         | 85.4         | 86.5            |
| Alignment          | Arena-Hard                       | 60.8                            | 89.5         | 91.7         | 91.0            |
...
| Math &Text         | AIME'24                          | 72.6                            | 79.5         | 79.3         | 80.4            |
...
| Multilingual Tasks | INCLUDE                          | 68.0                            | 69.7         | 71.7         | 71.9            |
...
|----------------------------------|------------|-------------------|-------------------------|---------------|-----------------|
| Architecture # Activated Params  | Dense 14B  | Dense 27B         | Dense 32B               | Dense 14B 14B | MoE 3B 30B      |
|                                  |            | 27B               | 32B                     |               |                 |
| # Total Params                   | 14B        | 82.6              | 83.9                    | 82.0          |                 |
| MMLU-Redux GPQA-Diamond          | 85.3 56.1  | 42.4              | 49.5                    | 54.8          | 84.1 54.8       |
...
| Multi-IF                         | /          |                   |                         | 72.9          |                 |
| INCLUDE                          | 49.5 65.3  | 69.8 71.4         | 63.2 67.5               | 67.8          | 70.8 67.8       |
...
|----------------------------------|---------------------------------|---------------------------------|--------------|--------------|
| Architecture                     | Dense                           | Dense                           | Dense 4B     | Dense 8B     |
| # Activated Params               | 14B                             | 32B                             |              |              |
...
|---------------------------------|--------------------------|-------------------|------------------------|-------------------------|-------------|-------------|
| Architecture # Activated Params | Dense 8B                 | Dense 12B         | Dense 7B               | Dense 14B               | Dense 4B 4B | Dense 8B 8B |
| # Total Params                  | 8B                       | 12B               | 7B                     | 14B                     |             |             |
| MMLU-Redux                      | 61.7                     | 77.8              | 75.4                   | 80.0 45.5               | 77.3 41.7   | 79.5 39.3   |
| GPQA-Diamond C-Eval             | 32.8 52.0                | 40.9 61.1         | 36.4 76.2              | 78.0                    | 72.2        | 77.9        |
...
| MATH-500                        |                          | 85.6              | 77.6                   | 83.4                    | 84.8        | 87.4        |
| AIME'24                         | 54.8                     | 22.4              | 9.1                    | 15.2                    | 25.0        | 29.1        |
| AIME'25                         | 6.3 2.7                  | 18.8              | 12.1                   | 13.6                    | 19.1        | 20.9        |
...
|----------------------|-------------------------------------|----------------------------------|---------------------------------|--------------|--------------|
|                      | Architecture # Activated Params     | Dense                            | Dense                           | Dense 0.6B   | Dense 1.7B   |
|                      |                                     | 1.5B                             | 8B                              |              |              |
|                      |                                     | 1.5B                             | 8B                              | 0.6B         | 1.7B         |
|                      | # Total Params                      |                                  |                                 |              | 73.9         |
| Tasks                | MMLU-Redux                          | 45.4                             | 66.4                            | 55.6         |              |
| General              | GPQA-Diamond C-Eval                 | 33.8 27.1                        | 49.0 50.4                       | 27.9 50.4    | 40.1 68.1    |
|                      | LiveBench 2024-11-25                | 24.9                             | 40.6                            | 30.3         | 51.1         |
|                      | prompt                              |                                  |                                 | 59.2         |              |
| Alignment            | IFEval strict                       | 39.9                             | 59.0 17.6                       |              | 72.5         |
|                      | Arena-Hard                          | 4.5 5.00                         | 6.24                            | 8.5 6.10     | 43.1 7.60    |
| Tasks                | AlignBench v1.1 Creative Writing v3 | 16.4                             | 51.1                            | 30.6         | 48.0         |
|                      | WritingBench                        | 4.03                             | 5.42                            | 5.61         | 7.02         |
| Math &Text Reasoning | MATH-500                            | 83.9                             | 89.1                            | 77.6         | 93.4         |
|                      | AIME'24                             | 28.9                             | 50.4 27.8                       | 10.7 15.1    | 48.3 36.8    |
|                      | AIME'25                             | 22.8                             |                                 | 30.3         |              |
|                      | ZebraLogic                          | 4.9                              | 37.1                            |              | 63.2         |
|                      | AutoLogi                            | 19.1                             | 63.4                            | 61.6         | 83.2         |
| Agent&               | BFCL v3                             | 14.0                             | 21.5                            | 46.4         | 56.6         |
| Coding               | LiveCodeBench v5                    | 13.2                             | 42.5                            | 12.3         | 33.2         |
|                      | Multi-IF                            | 13.3                             | 27.0 34.5                       | 36.1 35.9    | 51.2 51.8    |
|                      | INCLUDE                             | 21.9                             | 40.1                            | 43.1         | 59.1         |
| Multilingual Tasks   | MMMLU 14 languages                  | 27.3                             | 13.2                            | 7.8          | 36.1         |
|                      | MT-AIME2024                         | 12.4                             |                                 |              |              |
...
| # Total Params          | 33.3             | 67.9         | 50.7                     | 64.4                   | 44.6         | 64.4         |
| MMLU-Redux GPQA-Diamond | 19.2             | 25.2         | 29.8                     | 30.3                   | 22.9         | 28.6         |
...
| AutoLogi                | 16.4             | 28.8         | 22.5                     | 29.9 50.4              | 37.4         | 59.8         |
| BFCL v3                 | 16.3             | 31.3         | 47.8                     |                        | 44.1         | 52.2         |
...
The Effectiveness of Thinking Budget To verify that Qwen3 can enhance its intelligence level by leveraging an increased thinking budget, we adjust the allocated thinking budget on four benchmarks across Mathematics, Coding, and STEM domains. The resulting scaling curves are presented in Figure 2, Qwen3demonstrates scalable and smooth performance improvements correlated to the allocated thinking budget. Moreover, we observe that if we further extend the output length beyond 32K, the model's performance is expected to improve further in the future. We leave this exploration as future work.
...
|--------------------------|---------------|---------------|-----------|--------------------|---------------|-----------------|-------------|
| Off-policy Distillation  | 55.0 (90.0)   | 42.8 (83.3)   |      92.4 |               42   |          86.4 |            55.6 | -           |
| + Reinforcement Learning | 67.6 (90.0)   | 55.5 (83.3)   |      94.8 |               52.9 |          86.9 |            61.3 | 17,920      |
| + On-policy Distillation | 74.4 ( 93.3 ) | 65.5 ( 86.7 ) |      97   |               60.3 |          88.3 |            63.3 | 1,800       |
...
|                     |                      | Stage 2 Reasoning RL   | Stage 3 Thinking Mode Fusion   | Stage 3 Thinking Mode Fusion   | Stage 4 General RL   | Stage 4 General RL   |
|---------------------|----------------------|------------------------|--------------------------------|--------------------------------|----------------------|----------------------|
...
| Instruction &Format | prompt               | 73.0                   | +5.4                           |                                |                      |                      |
| Following           | IFEval strict        |                        | 78.4 64.6                      | 78.4                           | 85.0 +6.6 73.0       | 83.2 +4.8 70.7 +5.5  |
| Following           | Multi-IF LengthCtrl* | 61.4 62.6              | +3.2 70.6 +8.0                 | 65.2 84.9                      | +8.4 73.5 +2.9       | 87.3 +2.4            |
| Following           | ThinkFollow*         | -                      | 88.7                           | 88.7                           | 98.9 +10.2           | 98.9 +10.2           |
...
| Model                | RULER   | RULER   | RULER   | RULER   | RULER   | RULER   | RULER   |
|----------------------|---------|---------|---------|---------|---------|---------|---------|
...
|---------------|----------------------|------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |       80.1 |      70   |      96.4 |    88.7 |        90   |       54.4 |      79.9 |
| Thinking Mode | QwQ-32B              |       70   |      75   |      81.8 |    84.5 |        76.7 |       52.2 |      73.4 |
| Thinking Mode | Qwen3-235B-A22B      |       74.2 |      76.2 |      89.1 |    86.7 |        86.7 |       57.3 |      78.4 |
| Thinking Mode | Qwen3-32B            |       74.7 |      68.8 |      90.9 |    82.8 |        76.7 |       51.8 |      74.3 |
| Thinking Mode | Qwen3-30B-A3B        |       74.9 |      71.2 |      80   |    81.9 |        76.7 |       48.5 |      72.2 |
| Thinking Mode | Qwen3-14B            |       76.2 |      67.5 |      83.6 |    81.1 |        73.3 |       50.3 |      72   |
| Thinking Mode | Qwen3-8B             |       74.1 |      70   |      78.2 |    79.2 |        70   |       43.7 |      69.2 |
| Thinking Mode | Qwen3-4B             |       69.1 |      68.8 |      72.7 |    75.7 |        66.7 |       42.3 |      65.9 |
| Thinking Mode | Qwen3-1.7B           |       56   |      55   |      72.7 |    64.5 |        46.7 |       30.2 |      54.2 |
| Thinking Mode | Qwen3-0.6B           |       39.2 |      42.5 |      54.5 |    48.8 |        13.3 |       14.3 |      35.4 |
|               | GPT-4o-2024-1120     |       67.5 |      52.5 |      89.1 |    80.6 |        10   |       15.5 |      52.5 |
|               | Gemma-3-27b-IT       |       73.5 |      57.5 |      89.1 |    77.7 |        30   |       22.4 |      58.4 |
|               | Qwen2.5-72B-Instruct |       66.7 |      61.3 |      80   |    80.1 |        20   |       18.8 |      54.5 |
|               | Qwen3-235B-A22B      |       71.7 |      66.2 |      83.6 |    83.7 |        33.3 |       29.5 |      61.3 |
|               | Qwen3-32B            |       72.1 |      65   |      83.6 |    80.4 |        26.7 |       24.7 |      58.8 |
|               | Qwen3-30B-A3B        |       72.1 |      53.8 |      85.5 |    78.3 |        33.3 |       25   |      58   |
|               | Qwen3-14B            |       76.2 |      63.7 |      78.2 |    77.4 |        40   |       25   |      60.1 |
|               | Qwen3-8B             |       73.1 |      50   |      80   |    73.7 |        16.7 |       21.3 |      52.5 |
|               | Qwen3-4B             |       65.8 |      50   |      60   |    68.3 |        13.3 |       17.3 |      45.8 |
|               | Qwen3-1.7B           |       47.9 |      43.8 |      50.9 |    54.3 |        10   |       11.6 |      36.4 |
...
|               | Model                |   Multi-IF |   MLogiQA |   INCLUDE |   MMMLU |   MT-AIME24 |   PolyMath |   Average |
|---------------|----------------------|------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |       80.5 |      73.8 |      85.7 |    88.3 |        80   |       52.8 |      76.8 |
| Thinking Mode | QwQ-32B              |       72.4 |      78.8 |      76.2 |    84   |        80   |       49.4 |      73.5 |
| Thinking Mode | Qwen3-235B-A22B      |       77.3 |      78.8 |      85.7 |    86.6 |        86.7 |       57.4 |      78.8 |
| Thinking Mode | Qwen3-32B            |       76.7 |      81.2 |      76.2 |    82.1 |        83.3 |       47.1 |      74.4 |
| Thinking Mode | Qwen3-30B-A3B        |       75.2 |      67.5 |      83.3 |    81   |        76.7 |       46.9 |      71.8 |
| Thinking Mode | Qwen3-14B            |       77.6 |      71.2 |      73.8 |    80.4 |        73.3 |       44.2 |      70.1 |
| Thinking Mode | Qwen3-8B             |       73.8 |      66.2 |      85.7 |    77.9 |        70   |       45.3 |      69.8 |
| Thinking Mode | Qwen3-4B             |       71.3 |      63.7 |      71.4 |    74.5 |        66.7 |       40.2 |      64.6 |
| Thinking Mode | Qwen3-1.7B           |       52.6 |      56.2 |      54.8 |    64.8 |        60   |       28.7 |      52.8 |
| Thinking Mode | Qwen3-0.6B           |       36.1 |      48.8 |      47.6 |    48.4 |         6.7 |       14   |      33.6 |
|               | GPT-4o-2024-1120     |       67.8 |      56.2 |      85.7 |    81.8 |        10   |       15.3 |      52.8 |
|               | Gemma-3-27b-IT       |       73.9 |      57.5 |      73.8 |    78.3 |        23.3 |       21.5 |      54.7 |
|               | Qwen2.5-72B-Instruct |       72.1 |      55   |      81   |    80.2 |        26.7 |       15.7 |      55.1 |
|               | Qwen3-235B-A22B      |       73.2 |      65   |      88.1 |    81.1 |        36.7 |       28.1 |      62   |
|               | Qwen3-32B            |       75.8 |      60   |      73.8 |    79.5 |        30   |       23   |      57   |
|               | Qwen3-30B-A3B        |       75.6 |      52.5 |      69   |    77.9 |        26.7 |       27.3 |      54.8 |
|               | Qwen3-14B            |       78.4 |      63.7 |      73.8 |    75.1 |        33.3 |       24.4 |      58.1 |
|               | Qwen3-8B             |       71.9 |      52.5 |      71.4 |    71.7 |        20   |       21.4 |      51.5 |
|               | Qwen3-4B             |       64.2 |      47.5 |      61.9 |    67.6 |        20   |       19.2 |      46.7 |
|               | Qwen3-1.7B           |       46.1 |      43.8 |      64.3 |    53.2 |         3.3 |       11.6 |      37   |
|               | Qwen3-0.6B           |       32.8 |      35   |      38.1 |    39.4 |         6.7 |        4.6 |      26.1 |
...
|               | Model                |   Multi-IF |   MLogiQA |   INCLUDE |   MMMLU |   MT-AIME24 |   PolyMath |   Average |
|---------------|----------------------|------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |       80.5 |      73.8 |      83.9 |    88.9 |        73.3 |       52.2 |      75.4 |
| Thinking Mode | QwQ-32B              |       70.5 |      70   |      80.4 |    84   |        80   |       48.7 |      72.3 |
| Thinking Mode | Qwen3-235B-A22B      |       73.6 |      78.8 |      78.6 |    86.2 |        86.7 |       58.3 |      77   |
| Thinking Mode | Qwen3-32B            |       74.1 |      76.2 |      76.8 |    82.6 |        80   |       52.4 |      73.7 |
| Thinking Mode | Qwen3-30B-A3B        |       76.1 |      71.2 |      71.4 |    81   |        76.7 |       49.3 |      71   |
| Thinking Mode | Qwen3-14B            |       77.3 |      68.8 |      75   |    81.6 |        83.3 |       46.7 |      72.1 |
| Thinking Mode | Qwen3-8B             |       73.9 |      67.5 |      75   |    78.6 |        56.7 |       44.8 |      66.1 |
| Thinking Mode | Qwen3-4B             |       70.6 |      62.5 |      71.4 |    75.1 |        73.3 |       44.2 |      66.2 |
| Thinking Mode | Qwen3-1.7B           |       55.6 |      60   |      53.6 |    64.6 |        46.7 |       28.2 |      51.4 |
| Thinking Mode | Qwen3-0.6B           |       38.7 |      33.8 |      42.9 |    47.5 |        10   |       12.7 |      30.9 |
|               | GPT-4o-2024-1120     |       66.8 |      57.5 |      78.6 |    80.7 |        10   |       15   |      51.4 |
|               | Gemma-3-27b-IT       |       72.9 |      55   |      75   |    77.1 |        33.3 |       20.9 |      55.7 |
|               | Qwen2.5-72B-Instruct |       68.8 |      55   |      71.4 |    82.2 |        23.3 |       11.3 |      52   |
|               | Qwen3-235B-A22B      |       72.5 |      67.5 |      82.1 |    83.5 |        33.3 |       28.3 |      61.2 |
|               | Qwen3-32B            |       71.1 |      61.3 |      73.2 |    80.6 |        30   |       23.9 |      56.7 |
|               | Qwen3-30B-A3B        |       72.3 |      47.5 |      67.9 |    77.8 |        26.7 |       24   |      52.7 |
|               | Qwen3-14B            |       75.5 |      58.8 |      75   |    76.5 |        26.7 |       25.8 |      56.4 |
|               | Qwen3-8B             |       71.9 |      56.2 |      71.4 |    72.9 |        20   |       19.7 |      52   |
|               | Qwen3-4B             |       66.1 |      50   |      73.2 |    66.7 |        10   |       18.1 |      47.4 |
...
|---------------|----------------------|------------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |       80.9 |     100   |    87.2 |        90   |       54.1 |      82.4 |
| Thinking Mode | QwQ-32B              |       71.2 |      96.4 |    84.9 |        76.7 |       49.3 |      75.7 |
| Thinking Mode | Qwen3-235B-A22B      |       73.7 |      96.4 |    85.7 |        80   |       57.4 |      78.6 |
| Thinking Mode | Qwen3-32B            |       76.6 |      90.9 |    81.6 |        80   |       49.7 |      75.8 |
| Thinking Mode | Qwen3-30B-A3B        |       75.9 |      94.5 |    81.9 |        80   |       48.1 |      76.1 |
| Thinking Mode | Qwen3-14B            |       79   |      94.5 |    80.2 |        70   |       47   |      74.1 |
| Thinking Mode | Qwen3-8B             |       74.6 |      89.1 |    77.5 |        76.7 |       46.1 |      72.8 |
| Thinking Mode | Qwen3-4B             |       69.8 |      83.6 |    74.4 |        76.7 |       44.5 |      69.8 |
| Thinking Mode | Qwen3-1.7B           |       54.6 |      74.5 |    64.2 |        53.3 |       29.6 |      55.2 |
| Thinking Mode | Qwen3-0.6B           |       37.8 |      45.5 |    45.9 |         6.7 |       13.3 |      29.8 |
|               | GPT-4o-2024-1120     |       67.6 |      98.2 |    80.7 |        13.3 |       15.2 |      55   |
|               | Gemma-3-27b-IT       |       74.6 |      90.9 |    78.4 |        23.3 |       20.5 |      57.5 |
|               | Qwen2.5-72B-Instruct |       67.2 |      94.5 |    80.7 |        16.7 |       16.7 |      55.2 |
|               | Qwen3-235B-A22B      |       72.9 |      92.7 |    82.6 |        33.3 |       28.6 |      62   |
|               | Qwen3-32B            |       71.4 |      92.7 |    79.5 |        30   |       23   |      59.3 |
|               | Qwen3-30B-A3B        |       73.9 |      87.3 |    77.7 |        33.3 |       24.8 |      59.4 |
|               | Qwen3-14B            |       75.8 |      89.1 |    75.7 |        26.7 |       27.6 |      59   |
|               | Qwen3-8B             |       72.1 |      85.5 |    72.9 |        13.3 |       23.8 |      53.5 |
|               | Qwen3-4B             |       63   |      78.2 |    67.8 |        23.3 |       19.3 |      50.3 |
|               | Qwen3-1.7B           |       46.1 |      70.9 |    53.4 |         6.7 |       11.9 |      37.8 |
|               | Qwen3-0.6B           |       35.1 |      43.6 |    39   |         0   |        4.5 |      24.4 |
...
|---------------|----------------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      75   |      89.3 |    87.8 |        76.7 |       52.6 |      76.3 |
| Thinking Mode | QwQ-32B              |      75   |      67.9 |    81.8 |        80   |       41.3 |      69.2 |
| Thinking Mode | Qwen3-235B-A22B      |      80   |      71.4 |    83.6 |        76.7 |       53.7 |      73.1 |
| Thinking Mode | Qwen3-32B            |      66.2 |      73.2 |    80.1 |        86.7 |       47   |      70.6 |
| Thinking Mode | Qwen3-30B-A3B        |      66.2 |      66.1 |    77.2 |        83.3 |       47.3 |      68   |
| Thinking Mode | Qwen3-14B            |      71.2 |      67.9 |    77.4 |        83.3 |       46.6 |      69.3 |
| Thinking Mode | Qwen3-8B             |      65   |      67.9 |    74.4 |        76.7 |       44.9 |      65.8 |
| Thinking Mode | Qwen3-4B             |      62.5 |      55.4 |    67.7 |        66.7 |       41.2 |      58.7 |
| Thinking Mode | Qwen3-1.7B           |      55   |      44.6 |    53.2 |        36.7 |       25.8 |      43.1 |
| Thinking Mode | Qwen3-0.6B           |      40   |      41.1 |    38.9 |        10   |       11.7 |      28.3 |
...
|               | Qwen3-235B-A22B      |      66.2 |      67.9 |    79.5 |        40   |       28.2 |      56.4 |
|               | Qwen3-32B            |      55   |      69.6 |    75.7 |        23.3 |       25.4 |      49.8 |
|               | Qwen3-30B-A3B        |      48.8 |      64.3 |    71.6 |        30   |       22.6 |      47.5 |
|               | Qwen3-14B            |      52.5 |      60.7 |    69.5 |        23.3 |       23.5 |      45.9 |
|               | Qwen3-8B             |      45   |      58.9 |    64.6 |        13.3 |       16.4 |      39.6 |
|               | Qwen3-4B             |      52.5 |      42.9 |    56.7 |        13.3 |       15.3 |      36.1 |
|               | Qwen3-1.7B           |      31.2 |      37.5 |    43.6 |         3.3 |        9.4 |      25   |
|               | Qwen3-0.6B           |      40   |      39.3 |    35.4 |         0   |        3.8 |      23.7 |
...
|               | Model                |   MLogiQA |   INCLUDE |   MMMLU |   MT-AIME24 |   PolyMath |   Average |
|---------------|----------------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      72.5 |      74.5 |    83.8 |        83.3 |       55.4 |      73.9 |
| Thinking Mode | QwQ-32B              |      73.8 |      86.3 |    82.3 |        53.3 |       39.9 |      67.1 |
| Thinking Mode | Qwen3-235B-A22B      |      75   |      94.1 |    84.8 |        73.3 |       52.7 |      76   |
| Thinking Mode | Qwen3-32B            |      70   |      90.2 |    80.2 |        76.7 |       47.7 |      73   |
| Thinking Mode | Qwen3-30B-A3B        |      66.2 |      88.2 |    79.9 |        73.3 |       47.4 |      71   |
| Thinking Mode | Qwen3-14B            |      68.8 |      88.2 |    79.4 |        66.7 |       45.7 |      69.8 |
| Thinking Mode | Qwen3-8B             |      71.2 |      86.3 |    74.9 |        73.3 |       44.7 |      70.1 |
| Thinking Mode | Qwen3-4B             |      63.7 |      80.4 |    72.5 |        53.3 |       40.7 |      62.1 |
| Thinking Mode | Qwen3-1.7B           |      53.8 |      74.5 |    61.8 |        36.7 |       28.5 |      51.1 |
| Thinking Mode | Qwen3-0.6B           |      47.5 |      47.1 |    45.1 |        13.3 |       14.5 |      33.5 |
|               | GPT-4o-2024-1120     |      60   |      92.2 |    81.9 |        10   |       12.5 |      51.3 |
|               | Gemma-3-27b-IT       |      66.2 |      86.3 |    76.5 |        20   |       17.3 |      53.3 |
|               | Qwen2.5-72B-Instruct |      55   |      94.1 |    77.7 |        16.7 |       17.7 |      52.2 |
|               | Qwen3-235B-A22B      |      67.5 |      92.2 |    80.9 |        26.7 |       26.9 |      58.8 |
|               | Qwen3-32B            |      58.8 |      92.2 |    78   |        20   |       20.5 |      53.9 |
|               | Qwen3-30B-A3B        |      51.2 |      82.4 |    74.9 |        30   |       20.6 |      51.8 |
|               | Qwen3-14B            |      55   |      84.3 |    73.8 |        33.3 |       19.8 |      53.2 |
|               | Qwen3-8B             |      47.5 |      82.4 |    69.9 |        20   |       18.5 |      47.7 |
|               | Qwen3-4B             |      46.2 |      76.5 |    64.8 |        13.3 |       15.1 |      43.2 |
|               | Qwen3-1.7B           |      40   |      68.6 |    46.3 |         3.3 |       11.6 |      34   |
...
|               | Model                |   MLogiQA |   INCLUDE |   MMMLU |   MT-AIME24 |   PolyMath |   Average |
|---------------|----------------------|-----------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      75   |        88 |    85.9 |        76.7 |       50   |      75.1 |
| Thinking Mode | QwQ-32B              |      76.2 |        72 |    81.8 |        60   |       40   |      66   |
| Thinking Mode | Qwen3-235B-A22B      |      71.2 |        80 |    84.7 |        80   |       55.7 |      74.3 |
| Thinking Mode | Qwen3-32B            |      71.2 |        74 |    79.2 |        80   |       48.5 |      70.6 |
| Thinking Mode | Qwen3-30B-A3B        |      68.8 |        72 |    78.6 |        76.7 |       46.6 |      68.5 |
| Thinking Mode | Qwen3-14B            |      67.5 |        74 |    79.6 |        76.7 |       46   |      68.8 |
| Thinking Mode | Qwen3-8B             |      60   |        80 |    74.7 |        76.7 |       42.3 |      66.7 |
| Thinking Mode | Qwen3-4B             |      66.2 |        74 |    68.8 |        70   |       40.6 |      63.9 |
| Thinking Mode | Qwen3-1.7B           |      53.8 |        66 |    57.8 |        43.3 |       25.2 |      49.2 |
| Thinking Mode | Qwen3-0.6B           |      33.8 |        52 |    41.5 |        13.3 |       11.8 |      30.5 |
|               | GPT-4o-2024-1120     |      63.7 |        80 |    80.5 |        13.3 |       12.9 |      50.1 |
|               | Gemma-3-27b-IT       |      58.8 |        76 |    75.9 |        20   |       18.3 |      49.8 |
|               | Qwen2.5-72B-Instruct |      58.8 |        68 |    76.7 |         6.7 |       17.7 |      45.6 |
|               | Qwen3-235B-A22B      |      63.7 |        76 |    79.8 |        33.3 |       27.9 |      56.1 |
|               | Qwen3-32B            |      60   |        74 |    77.2 |        26.7 |       21.2 |      51.8 |
|               | Qwen3-30B-A3B        |      52.5 |        72 |    72.5 |        16.7 |       20.7 |      46.9 |
|               | Qwen3-14B            |      52.5 |        68 |    73.3 |        20   |       18.7 |      46.5 |
|               | Qwen3-8B             |      52.5 |        76 |    66.5 |        23.3 |       16.3 |      46.9 |
|               | Qwen3-4B             |      46.2 |        74 |    59.9 |        13.3 |       16.6 |      42   |
|               | Qwen3-1.7B           |      48.8 |        58 |    46   |         6.7 |        9   |      33.7 |
|               | Qwen3-0.6B           |      40   |        52 |    36.9 |         0   |        5.5 |      26.9 |
...
|---------------|----------------------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      80   |    86.3 |        83.3 |       51.3 |      75.2 |
| Thinking Mode | QwQ-32B              |      76.4 |    83.7 |        73.3 |       47.3 |      70.2 |
| Thinking Mode | Qwen3-235B-A22B      |      80   |    87.2 |        80   |       53.5 |      75.2 |
| Thinking Mode | Qwen3-32B            |      80   |    82   |        76.7 |       45.6 |      71.1 |
| Thinking Mode | Qwen3-30B-A3B        |      81.8 |    80.4 |        80   |       44.9 |      71.8 |
| Thinking Mode | Qwen3-14B            |      78.2 |    79.6 |        70   |       45.3 |      68.3 |
| Thinking Mode | Qwen3-8B             |      72.7 |    77.7 |        70   |       43.8 |      66   |
| Thinking Mode | Qwen3-4B             |      70.9 |    72.3 |        66.7 |       41.2 |      62.8 |
| Thinking Mode | Qwen3-1.7B           |      63.6 |    61.2 |        36.7 |       26.8 |      47.1 |
| Thinking Mode | Qwen3-0.6B           |      36.4 |    46.6 |        10   |       12.6 |      26.4 |
|               | GPT-4o-2024-1120     |      80   |    81.1 |        10   |       14.7 |      46.4 |
|               | Gemma-3-27b-IT       |      76.4 |    75.9 |        13.3 |       22.6 |      47   |
|               | Qwen2.5-72B-Instruct |      74.5 |    78.8 |        10   |       16.6 |      45   |
|               | Qwen3-235B-A22B      |      81.8 |    81.9 |        33.3 |       27.5 |      56.1 |
|               | Qwen3-32B            |      81.8 |    77.2 |        23.3 |       24.3 |      51.6 |
|               | Qwen3-30B-A3B        |      70.9 |    76.4 |        30   |       25.9 |      50.8 |
|               | Qwen3-14B            |      70.9 |    74.1 |        26.7 |       24.6 |      49.1 |
|               | Qwen3-8B             |      78.2 |    69.6 |        20   |       21.6 |      47.4 |
|               | Qwen3-4B             |      67.3 |    66.5 |        13.3 |       19   |      41.5 |
|               | Qwen3-1.7B           |      52.7 |    49   |         3.3 |       10.8 |      29   |
|               | Qwen3-0.6B           |      52.7 |    40   |         3.3 |        5.1 |      25.3 |
...
|---------------|----------------------|------------|-----------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |       68.1 |      80.4 |        70   |       52.3 |      67.7 |
| Thinking Mode | QwQ-32B              |       61.2 |      73.2 |        76.7 |       43.6 |      63.7 |
| Thinking Mode | Qwen3-235B-A22B      |       62.2 |      80.4 |        80   |       53.1 |      68.9 |
| Thinking Mode | Qwen3-32B            |       62.5 |      73.2 |        63.3 |       46.5 |      61.4 |
| Thinking Mode | Qwen3-30B-A3B        |       60.7 |      76.8 |        73.3 |       45.4 |      64   |
| Thinking Mode | Qwen3-14B            |       63.6 |      80.4 |        66.7 |       46.4 |      64.3 |
| Thinking Mode | Qwen3-8B             |       62.9 |      69.6 |        63.3 |       37.7 |      58.4 |
| Thinking Mode | Qwen3-4B             |       52.8 |      69.6 |        56.7 |       36.6 |      53.9 |
| Thinking Mode | Qwen3-1.7B           |       37.8 |      46.4 |        20   |       22.8 |      31.8 |
| Thinking Mode | Qwen3-0.6B           |       26.4 |      46.4 |         3.3 |        7   |      20.8 |
|               | GPT-4o-2024-1120     |       52   |      80.4 |        20   |       13.7 |      41.5 |
|               | Gemma-3-27b-IT       |       57.3 |      71.4 |        23.3 |       21.6 |      43.4 |
|               | Qwen2.5-72B-Instruct |       54.1 |      67.9 |        20   |       13.3 |      38.8 |
|               | Qwen3-235B-A22B      |       56.7 |      75   |        40   |       26.1 |      49.4 |
|               | Qwen3-32B            |       58.6 |      71.4 |        30   |       23.3 |      45.8 |
|               | Qwen3-30B-A3B        |       58   |      73.2 |        30   |       21.1 |      45.6 |
|               | Qwen3-14B            |       60.3 |      71.4 |        26.7 |       24.2 |      45.6 |
|               | Qwen3-8B             |       59.3 |      58.9 |        20   |       22.8 |      40.2 |
|               | Qwen3-4B             |       46.1 |      58.9 |        13.3 |       17.8 |      34   |
|               | Qwen3-1.7B           |       34.8 |      41.1 |         3.3 |       13.2 |      23.1 |
|               | Qwen3-0.6B           |       25.5 |      46.4 |         0   |        5.8 |      19.4 |
...
|---------------|----------------------|-----------|-----------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      72.5 |      89.1 |        70   |       52.1 |      70.9 |
| Thinking Mode | QwQ-32B              |      71.2 |      69.1 |        70   |       49.2 |      64.9 |
| Thinking Mode | Qwen3-235B-A22B      |      75   |      87.3 |        83.3 |       55.1 |      75.2 |
| Thinking Mode | Qwen3-32B            |      67.5 |      81.8 |        83.3 |       44   |      69.2 |
| Thinking Mode | Qwen3-30B-A3B        |      68.8 |      78.2 |        76.7 |       46.1 |      67.4 |
| Thinking Mode | Qwen3-14B            |      72.5 |      72.7 |        73.3 |       45.8 |      66.1 |
| Thinking Mode | Qwen3-8B             |      65   |      72.7 |        73.3 |       42.9 |      63.5 |
| Thinking Mode | Qwen3-4B             |      68.8 |      63.6 |        60   |       42.2 |      58.6 |
| Thinking Mode | Qwen3-1.7B           |      52.5 |      61.8 |        30   |       26.9 |      42.8 |
| Thinking Mode | Qwen3-0.6B           |      33.8 |      38.2 |         6.7 |        9.8 |      22.1 |
|               | GPT-4o-2024-1120     |      57.5 |      81.8 |        10   |       13   |      40.6 |
|               | Gemma-3-27b-IT       |      52.5 |      74.5 |        33.3 |       20.6 |      45.2 |
|               | Qwen2.5-72B-Instruct |      61.3 |      72.7 |        26.7 |       18.6 |      44.8 |
|               | Qwen3-235B-A22B      |      70   |      83.6 |        36.7 |       27.1 |      54.4 |
|               | Qwen3-32B            |      60   |      81.8 |        23.3 |       21.8 |      46.7 |
|               | Qwen3-30B-A3B        |      52.5 |      81.8 |        20   |       24.7 |      44.8 |
|               | Qwen3-14B            |      63.7 |      67.3 |        20   |       21.6 |      43.2 |
|               | Qwen3-8B             |      48.8 |      65.5 |        20   |       19.1 |      38.4 |
|               | Qwen3-4B             |      48.8 |      65.5 |        20   |       19   |      38.3 |
|               | Qwen3-1.7B           |      36.2 |      60   |         3.3 |       10.9 |      27.6 |
|               | Qwen3-0.6B           |      30   |      36.4 |         3.3 |        3.9 |      18.4 |
...
|               | Model                |   INCLUDE |   MMMLU |   MT-AIME24 |   PolyMath |   Average |
|---------------|----------------------|-----------|---------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      50   |    85.6 |        86.7 |       53.8 |      69   |
| Thinking Mode | QwQ-32B              |      57.1 |    83.8 |        76.7 |       51   |      67.2 |
| Thinking Mode | Qwen3-235B-A22B      |      71.4 |    86   |        83.3 |       55.4 |      74   |
| Thinking Mode | Qwen3-32B            |      64.3 |    81.9 |        86.7 |       48.1 |      70.2 |
| Thinking Mode | Qwen3-30B-A3B        |      64.3 |    81.9 |        80   |       46.6 |      68.2 |
| Thinking Mode | Qwen3-14B            |      57.1 |    80.9 |        70   |       48.1 |      64   |
| Thinking Mode | Qwen3-8B             |      64.3 |    78.1 |        66.7 |       43.6 |      63.2 |
| Thinking Mode | Qwen3-4B             |      57.1 |    74   |        73.3 |       43.1 |      61.9 |
| Thinking Mode | Qwen3-1.7B           |      64.3 |    63.4 |        36.7 |       26.8 |      47.8 |
| Thinking Mode | Qwen3-0.6B           |      57.1 |    47.6 |        10   |       13.7 |      32.1 |
|               | GPT-4o-2024-1120     |      57.1 |    80.4 |        10   |       13.5 |      40.2 |
|               | Gemma-3-27b-IT       |      57.1 |    76.1 |        26.7 |       20.2 |      45   |
|               | Qwen2.5-72B-Instruct |      64.3 |    79.9 |        16.7 |       19.3 |      45   |
|               | Qwen3-235B-A22B      |      71.4 |    81.7 |        40   |       25.9 |      54.8 |
|               | Qwen3-32B            |      57.1 |    77.2 |        30   |       21.9 |      46.6 |
|               | Qwen3-30B-A3B        |      57.1 |    77.7 |        23.3 |       25.2 |      45.8 |
|               | Qwen3-14B            |      57.1 |    76   |        30   |       24.5 |      46.9 |
|               | Qwen3-8B             |      64.3 |    70.8 |        20   |       19.9 |      43.8 |
|               | Qwen3-4B             |      64.3 |    66   |        26.7 |       16.4 |      43.4 |
|               | Qwen3-1.7B           |      42.9 |    53.2 |        10   |       10.6 |      29.2 |
...
|---------------|----------------------|-----------|-------------|------------|-----------|
| Thinking Mode | Gemini2.5-Pro        |      73.8 |        80   |       50.7 |      68.2 |
| Thinking Mode | QwQ-32B              |      75   |        60   |       41.3 |      58.8 |
| Thinking Mode | Qwen3-235B-A22B      |      73.8 |        86.7 |       53.6 |      71.4 |
| Thinking Mode | Qwen3-32B            |      73.8 |        76.7 |       46.9 |      65.8 |
| Thinking Mode | Qwen3-30B-A3B        |      63.7 |        80   |       45.2 |      63   |
| Thinking Mode | Qwen3-14B            |      65   |        76.7 |       44.4 |      62   |
| Thinking Mode | Qwen3-8B             |      68.8 |        70   |       41.3 |      60   |
| Thinking Mode | Qwen3-4B             |      60   |        60   |       39.4 |      53.1 |
| Thinking Mode | Qwen3-1.7B           |      48.8 |        33.3 |       23.7 |      35.3 |
| Thinking Mode | Qwen3-0.6B           |      33.8 |        13.3 |       11.4 |      19.5 |
|               | GPT-4o-2024-1120     |      52.5 |        10   |       11.9 |      24.8 |
|               | Gemma-3-27b-IT       |      50   |        16.7 |       19   |      28.6 |
|               | Qwen2.5-72B-Instruct |      58.8 |         6.7 |       17.4 |      27.6 |
|               | Qwen3-235B-A22B      |      61.3 |        23.3 |       27.6 |      37.4 |
|               | Qwen3-32B            |      61.3 |        13.3 |       22.2 |      32.3 |
|               | Qwen3-30B-A3B        |      50   |        30   |       22.3 |      34.1 |
|               | Qwen3-14B            |      47.5 |        23.3 |       22.1 |      31   |
|               | Qwen3-8B             |      42.5 |        10   |       17.2 |      23.2 |
|               | Qwen3-4B             |      43.8 |        13.3 |       16.1 |      24.4 |
|               | Qwen3-1.7B           |      42.5 |         6.7 |        9.5 |      19.6 |
|               | Qwen3-0.6B           |      37.5 |         0   |        3.6 |      13.7 |
...
|---------------------------|------------------|-----------------|-----------------|----------------|-------------|----------|--------------|----------|-----------------|---------|
| Gemma-3-27B-IT            |             89.2 |            86.3 |            85.9 |           84.1 |        83.5 |     86.8 |         81   |     91   |            86.5 |    87   |
| Qwen2.5-32B-Instruct      |             85.5 |            82.3 |            80.4 |           70.6 |        67.8 |     80.8 |         74.5 |     87   |            79   |    72.6 |
| QwQ-32B                   |             86.1 |            83.7 |            81.9 |           71.3 |        69.3 |     80.3 |         77   |     88   |            83   |    74   |
| Qwen3-32B (Thinking)      |             90.7 |            89.7 |            84.8 |           86.7 |        84.5 |     89.3 |         83.5 |     91.3 |            88   |    83.1 |
| Qwen3-32B (Non-thinking)  |             89.1 |            88   |            82.3 |           83.7 |        84   |     85   |         85   |     88.7 |            88   |    81.3 |
| Gemma-3-12B-IT            |             85.8 |            83.3 |            83.4 |           79.3 |        79   |     82.8 |         77.5 |     89   |            83   |    81.6 |
| Qwen2.5-14B-Instruct      |             82.7 |            78.9 |            80.4 |           69.1 |        66.2 |     74.2 |         72.2 |     83.9 |            77.9 |    70.4 |
| Qwen3-14B (Thinking)      |             88.6 |            87.3 |            82.4 |           82.4 |        81   |     83.8 |         83.5 |     91   |            82.5 |    81.7 |
| Qwen3-14B (Non-thinking)  |             87.4 |            82.7 |            80.1 |           80.7 |        78   |     81.8 |         80.5 |     87.7 |            81.5 |    77   |
| Gemma-3-4B-IT             |             71.8 |            72   |            63.5 |           61.7 |        64.8 |     64   |         61.5 |     70.7 |            71   |    62.6 |
| Qwen2.5-3B-Instruct       |             58   |            62.3 |            57.2 |           47.9 |        36.9 |     45.1 |         49.8 |     50.6 |            56.8 |    48.4 |
| Qwen3-4B (Thinking)       |             82.2 |            77.7 |            74.1 |           73   |        74.3 |     76.3 |         68.5 |     83   |            74.5 |    67.9 |
| Qwen3-4B (Non-thinking)   |             76   |            77   |            65.6 |           65.6 |        65.5 |     64   |         60.5 |     74   |            74   |    61   |
| Gemma-3-1B-IT             |             36.5 |            36   |            30   |           29.1 |        28.8 |     27.3 |         28   |     32.7 |            33   |    30.9 |
| Qwen2.5-1.5B-Instruct     |             41.5 |            43   |            39.6 |           34.8 |        28.6 |     29.7 |         39.4 |     33.8 |            42   |    36   |
| Qwen3-1.7B (Thinking)     |             69.7 |            66   |            59.4 |           58.6 |        52.8 |     57.8 |         53.5 |     70.3 |            63.5 |    53.4 |
| Qwen3-1.7B (Non-thinking) |             58.8 |            62.7 |            50.8 |           53   |        43.3 |     48   |         46   |     54.3 |            54   |    43.9 |
...
- AIME. AIME problems and solutions, 2025. URL https://artofproblemsolving.com/wiki/index.p hp/AIME Problems and Solutions .
...
- Lucas Bandarkar, Davis Liang, Benjamin Muller, Mikel Artetxe, Satya Narayan Shukla, Donald Husa, NamanGoyal, Abhinandan Krishnan, Luke Zettlemoyer, and Madian Khabsa. The Belebele benchmark: Aparallel reading comprehension dataset in 122 language variants. CoRR , abs/2308.16884, 2023.
...
- OpenAI. Learning to reason with LLMs, 2024. URL https://openai.com/index/learning-to-reaso n-with-llms/ .
- OpenAI. Introducing openai o3 and o4-mini, 2025. URL https://openai.com/index/introducing-o 3-and-o4-mini/ .
...
Example:
```markdown
![Description based on OCR](../media/doc_1769229613_84a5e2b1_arxiv_2505.09388.pdf) <!-- TODO: Broken image path -->
