...
**File:** doc_1770108589_54d9168a_arxiv_2601.21204.pdf
**Original:** arxiv_2601.21204.pdf
**Received:** 1770108589
...
1 Corresponding authors: zhangjiaqi39@meituan.com, peipeng@meituan.com

## Contents

| 1 Introduction   | 1 Introduction                                       | 1 Introduction                                                                         |   3 |
|------------------|------------------------------------------------------|----------------------------------------------------------------------------------------|-----|
| 2                | N-gram Embedding Layer                               | N-gram Embedding Layer                                                                 |   3 |
| 3                | Comparative Analysis of Expert and Embedding Scaling | Comparative Analysis of Expert and Embedding Scaling                                   |   4 |
...
| 4                | Efficient Inference                                  | Efficient Inference                                                                    |   9 |
...
| 5                | Integration with Per-Layer Embedding                 | Integration with Per-Layer Embedding                                                   |  11 |
|                  | 5.1                                                  | Per-Layer Embedding . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .    |  11 |
|                  | 5.2                                                  | Per-Layer N-gram Embedding . . . . . . . . . . . . . . . . . . . . . . . . . . .       |  11 |
|                  | 5.3                                                  | Empirical Comparison . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .     |  11 |
| 6                | LongCat-Flash-Lite                                   | LongCat-Flash-Lite                                                                     |  11 |
...
| 7                | Conclusions                                          | Conclusions                                                                            |  15 |
| 8                | Acknowledgement                                      | Acknowledgement                                                                        |  15 |
...
where E 0 ∈ R V 0 × D is the original base embedding table with hidden size D , E n ∈ R V n × D is the expanded embedding table, N denotes the maximum n-gram order and H n denotes the hash mapping function. We use the polynomial rolling hash funciton:
...
where E n,k ∈ R V n,k × D/ (( N -1) K ) is a sub-table and W n,k ∈ R D × D/ (( N -1) K ) is the linear projection matrix. By setting the hidden size of sub-tables to be inversely proportional to the number of sub-tables, this design ensures that the parameter count of N-gram Embedding remains invariant with respect to N and K .
...
1 We utilize the ratio of total parameters to activated parameters on the x-axis as a proxy for sparsity.
...
2 For large N , numerical overflow during hash computation can be circumvented by applying the modulus operation prior to exponentiation.
...
- Scaling Factor: Introducing a scaling factor (typically D ) to the embedding output to ensure a sufficient contribution to the forward pass.
...
PLE is applied in Google DeepMind [2024] and further studied in Sadhukhan et al. [2026]. PLE directly substitutes the output of up-projection matrix in the SwiGLU module with the embedding output, which is the most efficient method for injecting embedding information in our experiments. Let x ( l ) i be the i -th input vector of the FFN module in layer l , the FFN output with PLE can be formalized as follows

<!-- formula-not-decoded -->

where W ( l ) d and W ( l ) g denote the down-projection and gate-projection matrices of layer l respectively, and E ( l ) 0 is the embedding table of layer l , with identical shape to the base embedding table in Eq. 1.
...
where e ( l ) i is computed according to Eq. 3, with layer-specific embedding table and projection matrix.
...
|---------|---------------------|-----------------------------------|----------------------------|
| General | MMLU MMLU-pro CEval | 64.81 34.43 64.09                 | 64.01 35.89 67.21          |
| General | CMMLU               | 67.08                             | 69.55                      |
| General | BBH                 | 38.54                             | 43.67                      |
| General | GPQA                | 25.37                             | 29.66                      |
|         | DROP                | 47.92                             | 52.43                      |
|         | GSM8K               | 50.00                             | 50.50                      |
...
- Agentic Tool Use Tasks: τ 2 Bench [Barres et al., 2025], Vita Bench [He et al., 2025]. For τ 2 -Bench, we use a revised version 3 to perform our evaluation because the original version contains noisy data.
...
3 https://github.com/AGI-Eval-Official/tau2-bench-revised
...
|--------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| Architecture # Total Params # Activated Params                                                                           | MoE 48B 3B                                                                               | MoE 80B 3B                                                                               | - - -                                                                                    | MoE + NE 68.5B 2.9B ∼ 4.5B                                                               |
| Agentic Tool Use                                                                                                         | Agentic Tool Use                                                                         | Agentic Tool Use                                                                         | Agentic Tool Use                                                                         | Agentic Tool Use                                                                         |
| Tau2-Airline(avg@8) Tau2-Retail(avg@8) Tau2-Telecom(avg@8) VitaBench(avg@4)                                              | 44.00 18.86 15.68 -                                                                      | 45.5* 57.3* 13.2* 5.80                                                                   | 35.00 37.50 21.93 4.50                                                                   | 58.00 73.10 72.80 7.00                                                                   |
| SWE-Bench(acc) TerminalBench(acc) SWE-Bench Multiligual PRDBench GPQA-Diamond(avg@16) MMLU(acc) MMLU-Pro(acc) CEval(acc) | 32.80 20.00 37.20 -                                                                      | 37.60 15.19 31.30 15.36                                                                  | 41.3* 20.00 - -                                                                          | 54.40 33.75 38.10 39.63                                                                  |
| General Domains                                                                                                          | General Domains                                                                          | General Domains                                                                          | General Domains                                                                          | General Domains                                                                          |
| CMMLU(acc)                                                                                                               | 69.89 79.91 67.22 78.48                                                                  |                                                                                          | 70.20* 84.68 78.95 75.16                                                                 |                                                                                          |
| 74.33 66.78 89.28 85.52 82.93 78.29 90.91 86.55 86.50 72.06 82.48 Mathematical Reasoning                                 | 74.33 66.78 89.28 85.52 82.93 78.29 90.91 86.55 86.50 72.06 82.48 Mathematical Reasoning | 74.33 66.78 89.28 85.52 82.93 78.29 90.91 86.55 86.50 72.06 82.48 Mathematical Reasoning | 74.33 66.78 89.28 85.52 82.93 78.29 90.91 86.55 86.50 72.06 82.48 Mathematical Reasoning | 74.33 66.78 89.28 85.52 82.93 78.29 90.91 86.55 86.50 72.06 82.48 Mathematical Reasoning |
| MATH500(acc) AIME24(avg@32) AIME25(avg@32)                                                                               | 76.26 94.20 70.52 59.58                                                                  | 98.00 81.35 68.44                                                                        | 95.20 63.33 50.1*                                                                        | 96.80 72.19 63.23                                                                        |
...
Example:
```markdown
![Description based on OCR](../media/doc_1770108589_54d9168a_arxiv_2601.21204.pdf) <!-- TODO: Broken image path -->
