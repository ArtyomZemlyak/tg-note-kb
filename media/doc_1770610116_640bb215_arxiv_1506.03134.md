...
**File:** doc_1770610116_640bb215_arxiv_1506.03134.pdf
**Original:** arxiv_1506.03134.pdf
**Received:** 1770610116
...
Meire Fortunato ∗ Department of Mathematics, UC Berkeley
...
Here P = { P 1 , . . . , P n } is a sequence of n vectors and C P = { C 1 , . . . , C m ( P ) } is a sequence of m ( P ) indices, each between 1 and n .
...
As in [1], we use an Long Short Term Memory (LSTM) [11] to model p θ ( C i | C 1 , . . . , C i -1 , P ; θ ) . The RNN is fed P i at each time step, i , until the end of the input sequence is reached, at which time a special symbol, ⇒ is input to the model. The model then switches to the generation mode until the network encounters the special symbol ⇐ , which represents termination of the output sequence.
...
For notation purposes, let us define the encoder and decoder hidden states as ( e 1 , . . . , e n ) and ( d 1 , . . . , d m ( P ) ) , respectively. For the LSTM RNNs, we use the state after the output gate has been component-wise multiplied by the cell activations. We compute the attention vector at each output time i as follows:
...
1 We release all the datasets at http://goo.gl/NDcOIG .

<!-- image -->

(a) Input P = { P 1 , . . . , P 10 } , and the output sequence C P = {⇒ , 2 , 4 , 3 , 5 , 6 , 7 , 2 , ⇐} representing its convex hull.

(b) Input P = { P 1 , . . . , P 5 } , and the output C P = {⇒ , (1 , 2 , 4) , (1 , 4 , 5) , (1 , 3 , 5) , (1 , 2 , 3) , ⇐} representing its Delaunay Triangulation.
...
In this example, the outputs C P = { C 1 , . . . , C m ( P ) } are the corresponding sequences representing the triangulation of the point set P . Each C i is a triple of integers from 1 to n corresponding to the position of triangle vertices in P or the beginning/end of sequence tokens. See Figure 2 (b).
...
The input/output pairs ( P , C P ) have a similar format as in the Convex Hull problem described in Section 3.1. P will be the cartesian coordinates representing the cities, which are chosen randomly in the [0 , 1] × [0 , 1] square. C P = { C 1 , . . . , C n } will be a permutation of integers from 1 to n representing the optimal path (or tour). For consistency, in the training dataset, we always start in the first city without loss of generality.
...
2 We choose C i = (1 , 2 , 4) instead of (2,4,1) or any other permutation.
...
Example:
```markdown
![Description based on OCR](../media/doc_1770610116_640bb215_arxiv_1506.03134.pdf) <!-- TODO: Broken image path -->
