...
**File:** doc_1771230470_e87a9ce8_arxiv_2512.24695.pdf
**Original:** arxiv_2512.24695.pdf
**Received:** 1771230470
...
fast-weight matrix M 𝑡 ∈ R 𝑑 out × 𝑑 key that serves as a short-term memory. A separate 'programmer' (slow net) maps each input 𝒙 𝑡 ∈ R 𝑑 in to query, key, and value vectors and updates the fast weights online. A basic (Hebbian/outer-product) FWP-often called vanilla FWP-update its parameters with:
...
or equivalently, minimizes ⟨ 𝒎 , 𝑷 𝑖 ∇L( 𝑊 𝑖 ; 𝒙 𝑖 )⟩ . Using gradient descent for updating the momentum results in the update rule of:

<!-- formula-not-decoded -->

where we also included a forget gate 𝛼 𝑖 + 1 for fogetting the past. This update rule can be viewed as preconditioning the momentum GD. Here, the momentum term with preconditioning is interpreted as an associative memory that learns

how to compress the mappings between 𝑷 𝑖 and the gradient term ∇L( 𝑊 𝑖 ; 𝒙 𝑖 ) . While any reasonable choice (e.g., random features) of preconditioning may also be used to improve the expressivity of the initial version of GD with momentum. Note that our discussion on preconditioning in Equation 39 is different from here and indicates that one can learn the preconditioner as a proper mapping, while the above formulation shows when using preconditioning, momentum term (as a memory for gradients) aims to map them to their mapping function.
...
Section 8, refer to this variant as Hope-Attention). Let the model have 𝐿 layer layers, 4 levels of MLP blocks in CMS with highest frequency of ˆ 𝑓 , and hidden dimension of 𝑑 in . On average, the update cost is for O GLYPH&lt;16&gt; 1 ˆ 𝑓 × 𝐿 layer 5 × 𝑑 2 in GLYPH&lt;17&gt; of parameters, which consists of only a small number of parameters at each time.
...
where 𝒒 𝑡 = 𝒙 𝑡 𝑊 𝒒 is the only non-adaptive projection, 𝜂 𝑡 is the learning rate in optimization process, and 𝛼 𝑡 is the retention gate (forget gate or weight decay) in the optimization process. Note that, again, the initial states of all memories, i.e., M □ , 0
...
internal attentional bias of dot-product, 𝐿 2 , and 𝐿 𝑝 regression: i.e., TTT (Sun et al. 2024), Miras (Behrouz et al. 2025b), DLA (Behrouz et al. 2025a) and Titans (Behrouz et al. 2025c). Finally, we also compare with Transformers (Vaswani et al. 2017; Dubey et al. 2024) as well as the hybrid of attention and linear RNN, Samba (Ren et al. 2024).
...
- [159] Samuel J Gershman, Ila Fiete, and Kazuki Irie. 'Key-value memory in the brain'. In: Neuron 113.11 (2025), pp. 16941707.
...
- [165] Aonian Li, Bangwei Gong, Bo Yang, Boji Shan, Chang Liu, Cheng Zhu, Chunhao Zhang, Congchao Guo, Da Chen, Dong Li, et al. 'Minimax-01: Scaling foundation models with lightning attention'. In: arXiv preprint arXiv:2501.08313 (2025).
...
- [166] Saleh Momeni, Sahisnu Mazumder, Zixuan Ke, and Bing Liu. 'In-context continual learning assisted by an external continual learner'. In: Proceedings of the 31st International Conference on Computational Linguistics . 2025, pp. 72927306.
...
- [172] Chongjie Si, Debing Zhang, and Wei Shen. 'Adamuon: Adaptive muon optimizer'. In: arXiv preprint arXiv:2507.11005 (2025).
...
- [180] Yifan Zhang, Zhen Qin, and Quanquan Gu. 'Higher-order Linear Attention'. In: arXiv preprint arXiv:2510.27258 (2025).
...
one can interpret the momentum as a key or value-less associative memory (Behrouz et al. 2025b, see Section 5), where the gradient terms ∇ 𝑊 ℓ 𝑡 L GLYPH&lt;0&gt; 𝑊 ℓ 𝑡 ; 𝒙 𝑡 + 1 GLYPH&lt;1&gt; are compressed into the momentum. We expect from a powerful momentum term to perfectly memorize all the past gradients in the training process so it can better model the current update to the weights. To this end, one can start from defining a simple objective as:

<!-- formula-not-decoded -->

where 𝒈 ℓ 𝑡 + 1 = -∇ 𝑊 ℓ 𝑡 L GLYPH&lt;0&gt; 𝑊 ℓ 𝑡 ; 𝒙 𝑡 + 1 GLYPH&lt;1&gt; . This objective aims to find a momentum term that does not simply map the gradients to 1 (which also results in limited memory management in momentum), but it maps gradients to a global property of past data samples. The more expressive this global property is, the more accurately the momentum can incorporate the compressed information from past. Accordingly, the objective in Equation 101 admits an optimal associative memory that maps gradients to 𝑷 ℓ 𝑡 + 1 as:
...
where 𝒈 ℓ 𝑡 + 1 = -∇ 𝑊 ℓ 𝑡 L GLYPH&lt;0&gt; 𝑊 ℓ 𝑡 ; 𝒙 𝑡 + 1 GLYPH&lt;1&gt; . Similar to Equation 102, finding the optimal solution to the above objective is defined as:
...
Example:
```markdown
![Description based on OCR](../media/doc_1771230470_e87a9ce8_arxiv_2512.24695.pdf) <!-- TODO: Broken image path -->
