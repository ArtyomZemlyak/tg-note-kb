...
**File:** doc_1772875172_3acb0356_arxiv_2603.03975.pdf
**Original:** arxiv_2603.03975.pdf
**Received:** 1772875172
...
Jyoti Aneja, Michael Harrison, Neel Joshi, Tyler LaBonte, John Langford, Eduardo Salinas Microsoft Research https://aka.ms/Phi-4-reasoning-vision
...
Agrowing countertrend towards smaller models aims to boost efficiency, enabled by careful model design and data curation-a goal pioneered by the Phi (Gunasekar et al., 2023) family of models and furthered by Phi-4-reasoning-vision-15B. We specifically build on learnings from the Phi-4 (Abdin et al., 2024) and Phi-4-Reasoning (Abdin et al., 2025) language models and show how a multimodal model can be trained
...
Figure 2: Phi-4-reasoning-vision-15B presents a compelling option compared to existing models, pushing the Pareto frontier of the trade-off between accuracy and compute costs. We achieve competitive performance with much slower models that require more time and tokens, and higher accuracy than similarly fast models. These values were computed by averaging accuracy, time, and output token counts for a subset of 4 benchmarks: ChartQATEST, MathVistaMINI, MMMUVAL, and ScreenSpot\_v2.
...
- Dynamic S 2 (Liu et al., 2025a): similar to S 2 , but resizes to a rectangular resolution chosen to minimize distortion while admitting a tiling by 384 × 384 squares.
...
- Multi-crop with S 2 : similar to multi-crop but uses S 2 to broaden the receptive field, i.e., crops the image into (potentially overlapping) 1536 × 1536 squares, performs S 2 , and concatenates features on the token dimension.
...
| Dynamic resolution  |         3600 |        44.9 |         79.7 |             17.5 |      56   |
...
|---------------------|--------------|------------------|------------------|
| Learning rate       | 1 × 10 - 3   | 2 × 10 - 5       | 7 × 10 - 7       |
...
| Adam ( β 1 , β 2 )  | (0.9, 0.999) | (0.9, 0.95)      | (0.9, 0.95)      |
...
Training Data Composition by Category

Training with Mixed Non-Reasoning/Reasoning Data

<!-- image -->

GP I-40 Captions of images
...
|-----------|--------|-------|---------|------------|-------------|-----------------|
| 1M        | 150K   | 450K  | 1.6M    |       44   |        37.4 |            48.2 |
| 1M        | 150K   | 850K  | 2.0M    |       44.1 |        37.3 |            60   |
| 1M        | 450K   | 450K  | 1.9M    |       45.3 |        36   |            48.3 |
...
3. Reasoning LLM → Reasoning Multimodal Training: Areasoning base is used, but all multimodal data must include reasoning traces.
...
- HuggingFaceM4 Team. Docmatix: A massive dataset for document visual question answering. Hugging Face , 2024a.
- HuggingFaceM4 Team. WebSight: A synthetic dataset for improving code generation of screenshot-to-code models. https://huggingface.co/datasets/HuggingFaceM4/WebSight , 2024b.
- Badr Youbi Idrissi, Martin Arjovsky, Mohammad Pezeshki, and David Lopez-Paz. Simple data balancing achieves competitive worst-group-accuracy, 2022. URL https://arxiv.org/abs/2110.14503 .
...
- Miaosen Zhang, Ziqiang Xu, Jialiang Zhu, Qi Dai, Kai Qiu, Yifan Yang, Chong Luo, Tianyi Chen, Justin Wagle, Tim Franklin, and Baining Guo. Phi-ground tech report: Advancing perception in gui grounding, 2025. URL https://arxiv.org/abs/2507.23779 .
...
| Stage 2: Single-Image Instruction Tuning    | Caption Diagram &ChartQA DocumentQA Object Detection OCR Perception Text VQA Math: OCR Math: Problem CUA: General CUA: Grounding CUA: HTML RAI | Bunny (He et al., 2024) Recaptioned, Pixmo (Deitke et al., 2024), LLaVA-OneVision (Li et al., 2024a) LLaVA-OneVision (Li et al., 2024a), CoSyn (Yang et al., 2025) Docmatix (HuggingFaceM4 Team, 2024a), LLaVA-OneVision (Li et al., 2024a) Open Images (Kuznetsova et al., 2020) LLaVA-OneVision (Li et al., 2024a), IAM (Marti and Bunke, 2002) LLaVA-OneVision (Li et al., 2024a), VisOnlyQA (Kamoi et al., 2025) NuminaMath (AI-MO Team, 2024), OpenThoughts (Guha et al., 2025) Bunny (He et al., 2024), LLaVA-OneVision (Li et al., 2024a), LLaVA-NeXT (Li et al., 2024a), ShareGPT4V (Chen et al., 2023), Pixmo (Deitke et al., 2024) NuminaMath (AI-MO Team, 2024) LLaVA-OneVision (Li et al., 2024a), NuminaMath (AI-MO Team, 2024), MMR1(Leng et al., 2025), Eedi (Eedi, 2024) AGUVis (Xu et al., 2025), MultiUI (Liu et al., 2024a), Pixmo (Deitke et al., 2024), CoSyn (Yang et al., 2025), BigDocs (Rodriguez et al., 2025) PhiGround (Zhang et al., 2025), SeeClick (Cheng et al., 2024) WebSight (HuggingFaceM4 Team, 2024b) Hateful Memes (Kiela et al., 2021), Think-in-Safety (Lou et al., 2025), WildGuard (Han et al., 2024) |
| Stage 3: Long Context, Mulit-Image, and RAI | Caption DocumentQA VQA Math &Science CUA RAI                                                                                                   | Bunny (He et al., 2024) Docmatix (HuggingFaceM4 Team, 2024a) M4-Instruct (Li et al., 2024b) VisualWebInstruct (Jia et al., 2025) AGUVis (Xu et al., 2025) Hateful Memes (Kiela et al., 2021), VLGuard (Zong et al., 2024), Think-in-Safety (Lou et al., 2025), WildGuard (Han et al., 2024)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
Example:
```markdown
![Description based on OCR](../media/doc_1772875172_3acb0356_arxiv_2603.03975.pdf) <!-- TODO: Broken image path -->
