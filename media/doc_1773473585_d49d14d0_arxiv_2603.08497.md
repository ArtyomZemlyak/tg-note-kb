...
**File:** doc_1773473585_d49d14d0_arxiv_2603.08497.pdf
**Original:** arxiv_2603.08497.pdf
**Received:** 1773473585
...
Keywords: Vision-Language Models, Typographic Perception, Font Recognition, Visual Understanding Benchmark, Typography Blindness
...
https://henggg.cn/FontBlind/

/github Github Repo:

https://github.com/hengzzzhou/FontBlind

/envelope Contact:

hengzzzhou@gmail.com
...
This disconnect reveals a fundamental gap in current VLMs. Typography is the visual voice of written language: font choices establish hierarchies, convey tone, and encode meaning beyond the words themselves [Bringhurst, 2004, Garfield, 2012, Lupton, 2010]. Yet existing VLM benchmarks focus on what is in an image, such as objects, scenes, and text content [Bai et al., 2023, Chen et al., 2024b, Liu et al., 2023, 2024a,c, Wang et al., 2024], while neglecting how things look. This limitation has concrete consequences: document analysis systems must distinguish headings from body text by font size; accessibility tools must detect emphasis for screen readers; design verification systems must confirm brand-consistent font usage. Each application depends on typographic perception that current VLMs lack.
...
## 3.1 Font Registry and Data Generation
...
Table 1. Main results on FontBench. We report accuracy (%) for each property, overall, by difficulty level, and by writing system. Best and second best results are highlighted. Random baseline is 25%.

|                      | Per Property   | Per Property   | Per Property   | Per Property   |         | By Difficulty   | By Difficulty   | By Difficulty   | By Script   | By Script   | By Script   |
|----------------------|----------------|----------------|----------------|----------------|---------|-----------------|-----------------|-----------------|-------------|-------------|-------------|
...
## 5.2 The Capability-Fragility Trade-off

We stress-test three models under Gaussian noise ( σ ∈ { 10 , 50 } ), blur ( r ∈ { 1 , 4 } ), JPEG compression ( q ∈ { 75 , 10 } ), and rotation ( θ ∈ { 5 ◦ , 45 ◦ } ). The central finding, illustrated in Figure 4b, is a capability-fragility trade-off: the best clean-image model is the most vulnerable to degradation.
...
## 5.3 Cross-Benchmark Validation: Is This Real or Benchmark-Specific?

A critical validity question is whether FontBench's findings reflect genuine perceptual gaps or benchmark-specific artifacts. We evaluate all models on FRB [Li et al., 2025], an independently designed benchmark that tests 15 Latin fonts under two conditions: normal sentences (easy) and text that spells a conflicting font name (hard, creating a 'stroop

1 Baseline accuracies reported in this section correspond to robustness-set evaluations and may differ slightly from Table 1 owing to API response variability across evaluation runs.
...
effect' [Stroop, 1935]). Full per-model FRB results are provided in §E.1; fine-tuning transfer results are included in Table 2.
...
## 5.4 Attention Analysis: Where Models Look When They Fail
...
## 6 Closing the Gap: Fine-Tuning for Typographic Perception
...
Table 2. Fine-tuning results on FontBench (4-way MCQ, 25% baseline) and cross-benchmark transfer on FRB [Li et al., 2025] (15-way MCQ, 6.7% baseline). Best results highlighted. Full FRB results for all models are in §E.1.

|                                              | FontBench   | FontBench   | FontBench   | FontBench   | FontBench   | FRB     |
|----------------------------------------------|-------------|-------------|-------------|-------------|-------------|---------|
| Model                                        | Family      | Size        | Style       | Color       | Overall     | Overall |
| Baseline Models, Zero-Shot Qwen2.5-VL-7B     | 35.2        | 44.4        | 27.6        | 97.6        | 51.2        | 6.9     |
| Qwen3-VL-8B                                  | 36.8        | 39.2        | 28.8        | 99.6        | 51.1        | 9.6     |
| Fine-Tuned Models, LoRA Qwen2.5-VL-7B + LoRA | 46.0        | 66.4        | 27.6        | 98.0        | 59.5        | 9.6     |
| Qwen3-VL-8B + LoRA                           | 52.8        | 60.0        | 30.4        | 100.0       | 60.8        | 13.1    |
| Qwen2.5-VL-32B + LoRA                        | 43.6        | 34.4        | 31.2        | 94.8        | 51.0        | 13.3    |

## 6.1 Setup and Training Protocol
...
We apply LoRA adapters [Dettmers et al., 2023, Hu et al., 2022, Zhang et al., 2024] with rank r =16 , α =32 , and dropout 0.05 to the q/k/v/o\_proj attention matrices. Training uses AdamW with learning rate 2 × 10 -4 , warmup ratio 0.05, batch size 1 with 16 gradient accumulation steps, BF16 precision for the 7B and 8B models, and runs for 3 epochs. Each adapter trains only 0.2-0.3% of total parameters. Full hyperparameter details are provided in §D.

## 6.2 Results

The results in Table 2 confirm the prediction with striking precision. LoRA fine-tuning on just 3,000 synthetic samples lifts Qwen3-VL-8B from 51.1% to 60.8% and Qwen2.5-VL-7B from 51.2% to 59.5%, narrowing the gap to Gemini-3Flash from 15.6pp to 5.9pp. The 32B model improves only modestly by 3.5pp, though this is confounded by 4-bit NF4 quantization required to fit on a single GPU.
...
## 6.3 What Fine-Tuning Reveals About the Failure
...
Missing computational primitives. Font style's resistance across all model sizes, with gains of only 0.0-1.6pp, points to a qualitatively different bottleneck. Style perception requires comparing the observed stroke weight against an

Figure 6. Radar charts of per-property accuracy. (a) All models share a 'spiky' pattern with color ≫ others; Gemini-3-Flash dominates while GLM-4.5V collapses. (b) LoRA fine-tuning reshapes the capability profile: font size surpasses even Gemini-3-Flash after a 22.0pp gain for the 7B model, while the overall gap narrows to 5.9pp.

<!-- image -->

internal norm for each typeface, a relational computation that current patch-based encoders do not natively support. Architectural innovations such as contrastive modules or explicit style-comparison heads may be necessary.
...
- [1] Pravesh Agrawal, Szymon Antoniak, Emma Bou Hanna, Baptiste Bout, Devendra Chaplot, Jessica Chudnovsky, Diogo Costa, Baudouin De Monicault, Saurabh Garg, et al. Pixtral 12b. arXiv preprint arXiv:2410.07073 , 2024.
- [2] Stanislaw Antol, Aishwarya Agrawal, Jiasen Lu, Margaret Mitchell, Dhruv Batra, C. Lawrence Zitnick, and Devi Parikh. Vqa: Visual question answering. In International Conference on Computer Vision , pages 2425-2433, 2015.
...
- [4] Shuai Bai, Yuxuan Cai, Ruizhe Chen, Keqin Chen, Xionghui Chen, Zesen Cheng, Lianghao Deng, Wei Ding, Chang Gao, Chunjiang Ge, et al. Qwen3-vl technical report. arXiv preprint arXiv:2511.21631 , 2025a.
- [5] Shuai Bai, Keqin Chen, Xuejing Liu, Jialin Wang, Wenbin Ge, Sibo Song, Kai Dang, Peng Wang, Shijie Wang, Jun Tang, et al. Qwen2.5-vl technical report. arXiv preprint arXiv:2502.13923 , 2025b.
- [6] Darwin Bautista and Rowel Atienza. Scene text recognition with permuted autoregressive sequence models. In European Conference on Computer Vision , 2022.
- [7] Brent Berlin and Paul Kay. Basic Color Terms: Their Universality and Evolution . University of California Press, 1991.
- [8] Robert Bringhurst. The Elements of Typographic Style . Hartley &amp; Marks, 2004.
- [9] Zhe Chen, Weiyun Wang, Yue Cao, Yangzhou Liu, Zhangwei Gao, Erfei Cui, Jinguo Zhu, Shenglong Ye, Hao Tian, et al. Expanding performance boundaries of open-source multimodal models with model, data, and test-time scaling. arXiv preprint arXiv:2412.05271 , 2024a.
...
- [11] Tim Dettmers, Artidoro Pagnoni, Ari Holtzman, and Luke Zettlemoyer. Qlora: Efficient finetuning of quantized LLMs. Advances in Neural Information Processing Systems , 2023.
- [12] Alexey Dosovitskiy, Lucas Beyer, Alexander Kolesnikov, Dirk Weissenborn, Xiaohua Zhai, Thomas Unterthiner, Mostafa Dehghani, Matthias Minderer, Georg Heigold, Sylvain Gelly, et al. An image is worth 16x16 words: Transformers for image recognition at scale. In International Conference on Learning Representations , 2021.
- [13] Yuchen Fan, Chen Ling, Xin Zhong, Shuo Zhang, Heng Zhou, Yuchen Zhang, Mingyu Liang, Chengxing Xie, Ermo Hua, Zhizhou He, et al. Lfqa-e: Carefully benchmarking long-form qa evaluation. In The Fourteenth International Conference on Learning Representations , 2025.
...
- [15] Simon Garfield. Just My Type: A Book About Fonts . Profile Books, 2012.
- [16] Robert Geirhos, Patricia Rubisch, Claudio Michaelis, Matthias Bethge, Felix A Wichmann, and Wieland Brendel. Imagenet-trained CNNs are biased towards textures; increasing shape bias improves accuracy and robustness. In International Conference on Learning Representations , 2019.
- [17] Google Gemini Team. Gemini: A family of highly capable multimodal models. arXiv preprint arXiv:2312.11805 , 2023.

- [18] Yash Goyal, Tejas Khot, Douglas Summers-Stay, Dhruv Batra, and Devi Parikh. Making the v in vqa matter: Elevating the role of image understanding in visual question answering. In Conference on Computer Vision and Pattern Recognition , pages 6904-6913, 2017.
- [19] Edward J Hu, Yelong Shen, Phillip Wallis, Zeyuan Allen-Zhu, Yuanzhi Li, Shean Wang, Lu Wang, and Weizhu Chen. Lora: Low-rank adaptation of large language models. International Conference on Learning Representations , 2022.
- [20] Drew A. Hudson and Christopher D. Manning. Gqa: A new dataset for real-world visual reasoning and compositional question answering. In Conference on Computer Vision and Pattern Recognition , pages 6700-6709, 2019.
- [21] Keechul Jung. Neural network-based text location in color images. Pattern Recognition Letters , 22(14):1503-1515, 2001. doi: 10.1016/S0167-8655(01)00096-4.
- [22] Jonathan Krause, Michael Stark, Jia Deng, and Li Fei-Fei. 3d object representations for fine-grained categorization. In International Conference on Computer Vision Workshops , pages 554-561, 2013.
- [23] Minghao Li, Tengchao Lv, Jingye Chen, Lei Cui, Yijuan Lu, Dinei Florencio, Cha Zhang, Zhoujun Li, and Furu Wei. TrOCR: Transformer-based optical character recognition with pre-trained models. AAAI Conference on Artificial Intelligence , 2023.
- [24] Zhecheng Li, Guoxian Song, Yujun Cai, Zhen Xiong, Junsong Yuan, and Yiwei Wang. Texture or semantics? vision-language models get lost in font recognition. COLM , 2025.
- [25] Haotian Liu, Chunyuan Li, Qingyang Wu, and Yong Jae Lee. Visual instruction tuning. Advances in Neural Information Processing Systems , 36, 2023.
- [26] Haotian Liu, Chunyuan Li, Yuheng Li, and Yong Jae Lee. Improved baselines with visual instruction tuning. In Conference on Computer Vision and Pattern Recognition , 2024a.
- [27] Haotian Liu, Chunyuan Li, Yuheng Li, Bo Li, Yuanhan Zhang, Sheng Shen, and Yong Jae Lee. Llava-next: Improved reasoning, OCR, and world knowledge. In NeurIPS 2024 Workshop , 2024b.
- [28] Yuan Liu, Haodong Duan, Yuanhan Zhang, Bo Li, Songyang Zhang, Wangbo Zhao, Yike Yuan, Jiaqi Wang, Conghui He, Ziwei Liu, et al. MMBench: Is your multi-modal model an all-around player? In European Conference on Computer Vision , 2024c.
- [29] Yuliang Liu, Zhang Li, Mingxin Huang, Biao Yang, Wenwen Yu, Chunyuan Li, Xu-Cheng Yin, Cheng-Lin Liu, Lianwen Jin, and Xiang Bai. OCRBench: On the hidden mystery of OCR in large multimodal models. Science China Information Sciences , 67(12), 2024d. doi: 10.1007/s11432-024-4235-6.
- [30] Yuliang Liu, Biao Yang, Qiang Liu, Zhang Li, Zhiyin Ma, Shuo Zhang, and Xiang Bai. Textmonkey: An OCR-free large multimodal model for understanding document. arXiv preprint arXiv:2403.04473 , 2024e.
- [31] Pan Lu, Swaroop Mishra, Tony Xia, Liang Qiu, Kai-Wei Chang, Song-Chun Zhu, Oyvind Tafjord, Peter Clark, and Ashwin Kalyan. Learn to explain: Multimodal reasoning via thought chains for science question answering. Advances in Neural Information Processing Systems , 35:2507-2521, 2022.
...
- [33] Ellen Lupton. Thinking with Type: A Critical Guide for Designers, Writers, Editors, &amp; Students . Princeton Architectural Press, 2010.
- [34] Subhransu Maji, Esa Rahtu, Juho Kannala, Matthew Blaschko, and Andrea Vedaldi. Fine-grained visual classification of aircraft. Technical report, arXiv preprint arXiv:1306.5151, 2013.
- [35] Minesh Mathew, Dimosthenis Karatzas, and C.V. Jawahar. Docvqa: A dataset for vqa on document images. In IEEE/CVF Winter Conference on Applications of Computer Vision , pages 2200-2209, 2021.

- [36] OpenAI. Gpt-4 technical report. arXiv preprint arXiv:2303.08774 , 2023.
- [37] Alec Radford, Jong Wook Kim, Chris Hallacy, Aditya Ramesh, Gabriel Goh, Sandhini Agarwal, Girish Sastry, Amanda Askell, Pamela Mishkin, Jack Clark, et al. Learning transferable visual models from natural language supervision. International Conference on Machine Learning , pages 8748-8763, 2021.
- [38] Aaditya Singh, Adam Fry, Adam Perelman, et al. OpenAI GPT-5 system card. arXiv preprint arXiv:2601.03267 , 2025.
- [39] Amanpreet Singh, Vivek Natarajan, Meet Shah, Yu Jiang, Xinlei Chen, Dhruv Batra, Devi Parikh, and Marcus Rohrbach. Towards vqa models that can read. In Conference on Computer Vision and Pattern Recognition , pages 8317-8326, 2019.
- [40] J. Ridley Stroop. Studies of interference in serial verbal reactions. Journal of Experimental Psychology , 18(6): 643-662, 1935.
- [41] Maham Tanveer, Yizhi Wang, Ali Mahdavi-Amiri, and Hao Zhang. DS-Fusion: Artistic typography via discriminated and stylized diffusion. In International Conference on Computer Vision , 2023.
- [42] Yuki Tatsukawa, I-Chao Shen, Anran Qi, Yuki Koyama, Takeo Igarashi, and Ariel Shamir. Fontclip: A semantic typography visual-language model for multilingual font applications. Computer Graphics Forum , 43(2), 2024.
- [43] VTeam, Wenyi Hong, Wenmeng Yu, Xiaotao Gu, Guo Wang, et al. GLM-4.5V and GLM-4.1V-thinking: Towards versatile multimodal reasoning with scalable reinforcement learning, 2026.
- [44] Catherine Wah, Steve Branson, Peter Welinder, Pietro Perona, and Serge Belongie. The caltech-ucsd birds-2002011 dataset. Technical report, California Institute of Technology, 2011.
- [45] Peng Wang, Shuai Bai, Sinan Tan, Shijie Wang, Zhihao Fan, Jinze Bai, Keqin Chen, Xuejing Liu, Junyang Lin, Chang Zhou, and Jingren Zhou. Qwen2-vl: Enhancing vision-language model's perception of the world at any resolution. arXiv preprint arXiv:2409.12191 , 2024.
- [46] Yizhi Wang, Zhouhui Lian, Yingmin Tang, and Jianguo Xiao. Font recognition in natural images via transfer learning. In MultiMedia Modeling , pages 229-240. Springer International Publishing, 2018.
- [47] Zhangyang Wang, Jianchao Yang, Hailin Jin, Eli Shechtman, Aseem Agarwala, Jonathan Brandt, and Thomas S. Huang. Deepfont: Identify your font from an image. In ACM International Conference on Multimedia , pages 451-459, 2015.
- [48] Jiabo Ye, Anwen Hu, Haiyang Xu, Qinghao Ye, Ming Yan, Yuhao Dan, Chenlin Zhao, Guohai Xu, Chenliang Li, Junfeng Tian, Qian Qi, Ji Zhang, and Fei Huang. mplug-docowl: Modularized multimodal large language model for document understanding, 2023.
- [49] Weihao Yu, Zhengyuan Yang, Linjie Li, Jianfeng Wang, Kevin Lin, Zicheng Liu, Xinchao Wang, and Lijuan Wang. MM-Vet: Evaluating large multimodal models for integrated capabilities. In International Conference on Machine Learning , 2024.
...
- [51] Renrui Zhang, Jiaming Han, Chris Liu, Peng Gao, Aojun Zhou, Xiangfei Hu, Shilin Yan, Pan Lu, Hongsheng Li, and Yu Qiao. Llama-adapter: Efficient fine-tuning of language models with zero-init attention, 2024. URL https://arxiv.org/abs/2303.16199 .
- [52] Abdelwahab Zramdini and Rolf Ingold. Optical font recognition using typographical features. IEEE Transactions on Pattern Analysis and Machine Intelligence , 20(8):877-882, 1998.

̸

## Reading = Seeing: Diagnosing and Closing the Typography Gap in Vision-Language Models (Appendix)

| A Benchmark Construction   | A Benchmark Construction                  | A Benchmark Construction                  |   15 |
|----------------------------|-------------------------------------------|-------------------------------------------|------|
|                            | A.1                                       | Font Registry and Sample Gallery          |   15 |
|                            | A.2                                       | Data Generation Details . . . . . .       |   15 |
|                            | A.3                                       | Dataset Details . . . . . . . . . .       |   16 |
|                            | A.4                                       | Question Templates . . . . . . . .        |   17 |
| B                          | Evaluation Setup                          | Evaluation Setup                          |   17 |
|                            | B.1                                       | Model List . . . . . . . . . . . . .      |   17 |
|                            | B.2                                       | Implementation Details . . . . . .        |   17 |
|                            | B.3                                       | Compute Resources . . . . . . . .         |   18 |
| C                          | Complete Results and Statistical Analysis | Complete Results and Statistical Analysis |   18 |
|                            | C.1                                       | Complete Results . . . . . . . . .        |   18 |
|                            | C.2                                       | Statistical Analysis . . . . . . . .      |   19 |
|                            | C.3                                       | Perception Hierarchy Illustration .       |   20 |
|                            | C.4                                       | Accuracy Heatmap . . . . . . . .          |   20 |
| D                          | Fine-Tuning Details                       | Fine-Tuning Details                       |   20 |
|                            | D.1                                       | LoRA Configuration . . . . . . .          |   20 |
|                            | D.2                                       | Training Data . . . . . . . . . . .       |   21 |
| E                          | Cross-Benchmark Validation                | Cross-Benchmark Validation                |   21 |
|                            | E.1                                       | Full FRB Cross-Benchmark Results          |   21 |
|                            | E.2                                       | Stroop Effect Visualization . . . .       |   21 |
| F                          | Detailed Analysis                         | Detailed Analysis                         |   22 |
|                            | F.1                                       | Per-Property Detailed Analysis . .        |   22 |
|                            | F.2                                       | Scaling Paradox Details . . . . . .       |   23 |
|                            | F.3                                       | Error Analysis . . . . . . . . . . .      |   24 |
|                            | F.4                                       | Failure Case Analysis . . . . . . .       |   24 |
...
| G.1   | Limitations . . .   |   24 |
|-------|---------------------|------|
| G.2   | Future Directions   |   25 |
| G.3   | Reproducibility .   |   25 |

## A Benchmark Construction

## A.1 Font Registry and Sample Gallery

Table 3. Font categories in FontBench. The registry spans multiple typeface classifications and four writing systems.
...
| Monospace Display | Latin Latin | Courier New, Menlo American Typewriter                                                                 |
| CJK               | Chinese     | STHeiti, Songti SC                                                                                     |
| Arabic            | Arabic      | Al Nile, Baghdad Devanagari MT, Devanagari                                                             |
| Devanagari        | Devanagari  | SangamMN                                                                                               |

## Benchmark Sample

Figure 7. FontBench sample gallery. The four typographic dimensions evaluated: font family (note Arial vs. Helvetica are visually near-identical), font size (12-64pt), font style (regular/bold/italic/bold-italic), and font color. The rightmost column shows a representative question for each dimension.
...
Font registry F , text corpus T , target count N , difficulty distribution D
...
- 1: Initialize S ← ∅
- 2: for i = 1 to N do
- 3: Sample difficulty d ∼ D
- 4: Sample script s according to difficulty-script distribution
- 5: Sample font f ∼ F [ s ] respecting difficulty constraints
- 6: Sample text t ∼ T [ s ]
- 7: Sample size z ∈ { 12 , 16 , 20 , 24 , 32 , 40 , 48 , 64 }
- 8: Sample style y ∈ { regular , bold , italic , bold\_italic }
- 9: Sample color c ∈ { black , red , blue , green , . . . }
- 10: Compute background color b ensuring contrast with c
- 11: Render image I with parameters ( f, t, z, y, c, b )
- 12: Generate questions Q for each property
- 13: S ← S ∪ { ( I, Q, metadata ) }
- 14: end for
- 15: return S

## A.2.2 Font Size and Color Configuration

Tables 4 and 5 specify the eight font sizes and eight text colors used in the benchmark.

| Table 4. Font size configuration.   | Table 4. Font size configuration.   | Table 4. Font size configuration.   | Table 5. Font color palette.   | Table 5. Font color palette.   | Table 5. Font color palette.   |
|-------------------------------------|-------------------------------------|-------------------------------------|--------------------------------|--------------------------------|--------------------------------|
| Point Size                          | Bucket                              | Usage                               | Color                          | RGB                            | Hex                            |
| 12pt                                | Small                               | Body text                           | Black                          | (0, 0, 0)                      | #000000                        |
| 16pt                                | Small                               | Captions                            | Red                            | (220, 50, 50)                  | #DC3232                        |
| 20pt                                | Medium                              | Subheadings                         | Blue                           | (50, 50, 220)                  | #3232DC                        |
| 24pt                                | Medium                              | Subheadings                         | Green                          | (50, 150, 50)                  | #329632                        |
| 32pt                                | Large                               | Headings                            | Gray                           | (128, 128, 128)                | #808080                        |
| 40pt                                | Large                               | Titles                              | Orange                         | (230, 130, 30)                 | #E6821E                        |
| 48pt                                | XLarge                              | Display                             | Purple                         | (150, 50, 180)                 | #9632B4                        |
| 64pt                                | XLarge                              | Large display                       | Brown                          | (140, 90, 40)                  | #8C5A28                        |

## A.3 Dataset Details

## A.3.1 Complete Font List

Table 6 presents the complete list of 26 fonts included in FontBench.

Table 6. Complete font list organized by script and category. All fonts are available on macOS and selected for broad typographic diversity.

| Script     | Category                              | Fonts                                                                                                                                                                                                                    |
|------------|---------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Latin (20) | Serif Sans-Serif Monospace Slab Serif | Times New Roman, Georgia, Palatino, Baskerville, Didot, Cochin, Rock- well Arial, Arial Bold, Helvetica, Helvetica Neue, Verdana, Futura, Avenir, Gill Sans, Optima, Trebuchet MS Courier New, Menlo American Typewriter |
| CJK (2)    | Chinese                               | STHeiti, Songti SC                                                                                                                                                                                                       |
| Other (4)  | Arabic Devanagari                     | Al Nile, Baghdad Devanagari MT, Devanagari SangamMN                                                                                                                                                                      |
...
Font Family. 'What font family is used in this image?' / 'Identify the typeface shown in this text.' / 'Which font is used to render this text?' / 'What is the name of this font?'

Font Size. 'What is the approximate font size in this image?' / 'Estimate the point size of the displayed text.' / 'How large is the font in this image?' / 'What size is this text rendered at?'

Font Style. 'What style is the text rendered in?' / 'Is this text regular, bold, italic, or bold-italic?' / 'Identify the font style used here.' / 'What typographic style is applied to this text?'

Font Color. 'What color is the text in this image?' / 'Identify the font color.' / 'What is the color of the displayed text?' / 'Which color is used for this text?'
...
All models are accessed with temperature 0, max\_tokens 100, and default sampling parameters ( top\_p 1.0, no frequency or presence penalties).

Table 8. Summary of evaluated Vision-Language Models. Models span different sizes, architectures, and providers.
...
| Gemini-3-Flash       | Google               | Proprietary | Unknown           | API      |
| Gemini-3-Pro         | Google               | Proprietary | Unknown           | API      |
| Claude-Sonnet-4.6    | Anthropic            | Proprietary | Unknown           | API      |
| Doubao-Seed-1.6      | ByteDance            | Proprietary | Unknown           | API      |

## B.2.3 Prompt Template

```
Evaluation Prompt Template Look at this image and answer the following question. Question: {question} Options: A. {option_a} B. {option_b} C. {option_c} D. {option_d} Respond with only the letter (A, B, C, or D) of your answer.
```

## B.2.4 Answer Parsing
...
## C Complete Results and Statistical Analysis

## C.1 Complete Results

Table 9 reports per-property and per-difficulty accuracy for all evaluated models.

Table 9. Accuracy (%) by property and difficulty for all models.

| Model                | By Property   | By Property   | By Property   | By Property   | By Difficulty   | By Difficulty   | By Difficulty   | By Difficulty   |
|----------------------|---------------|---------------|---------------|---------------|-----------------|-----------------|-----------------|-----------------|
...
## C.2.1 Confidence Intervals

We compute 95% confidence intervals using the Wilson score interval for binomial proportions ( n =1000 questions per model).

Table 10. Overall accuracy with 95% confidence intervals ( n =1000 questions per model).
...
Using McNemar's test for pairwise model comparisons: Gemini-3-Flash is significantly better than all other models ( p &lt; 0 . 001 ). GPT-5.2 and Claude-Sonnet-4.6 are significantly better than middle-tier models ( p &lt; 0 . 05 ). Pairwise differences among the middle-tier cluster (Qwen3-VL-30B-A3B, Doubao-Seed-1.6, Qwen3-Max, Gemini-3-Pro, Qwen3-VL-32B, Qwen2.5-VL-7B, Qwen2.5-VL-72B, Qwen3-VL-8B) are not statistically significant ( p &gt; 0 . 05 ),

consistent with their overlapping confidence intervals. GLM-4.5V and Pixtral-12B are not significantly different from the 25% random baseline ( p &gt; 0 . 05 ).

## C.3 Perception Hierarchy Illustration
...
Figure 8. Perception hierarchy. Typographic properties map onto increasing visual feature complexity: color (0th-order pixel statistics, 89-100%) ≫ family (1st-order glyph shape, 26-81%) &gt; size (1st-order spatial extent, 22-52%) &gt; style (2nd-order relational comparison, 23-34%). Dashed line marks the 25% random-chance baseline.

<!-- image -->

## C.4 Accuracy Heatmap
...
```
lora_config = { "r": 16, "lora_alpha": 32, "lora_dropout": 0.05, "target_modules": ["q_proj", "k_proj", "v_proj", "o_proj"], "task_type": "CAUSAL_LM" } training_args = { "learning_rate": 2e-4, "num_epochs": 3, "batch_size": 1, "gradient_accumulation_steps": 16, "warmup_ratio": 0.05, "bf16": True , "gradient_checkpointing": True }
```

## Model × Property Accuracy (%)

Figure 9. Model × property accuracy heatmap. A clear perception hierarchy emerges: color is trivially solved, while style remains universally poor. Gemini-3-Flash leads across all properties; GLM-4.5V performs at near-random levels.

<!-- image -->

|                   |   Family |   Size |   Style |   Color |   Overall |
|-------------------|----------|--------|---------|---------|-----------|
| Gemini-3-Flash    |     80.8 |   52.4 |    33.6 |   100   |      66.7 |
| GPT-5.2           |     58.8 |   50   |    31.2 |    99.6 |      59.9 |
| Claude-Sonnet-4.6 |     64   |   44.8 |    28   |    97.6 |      58.6 |
| Doubao-Seed-1.6   |     44.8 |   44.4 |    30.4 |    98.8 |      54.6 |
| Qwen3-VL-30B-A3B  |     49.6 |   40.4 |    28   |    99.6 |      54.4 |
| Qwen3-Max         |     46   |   32.4 |    31.2 |    99.2 |      52.2 |
| Qwen3-VL-32B      |     42.4 |   37.2 |    26   |   100   |      51.4 |
| Qwen2.5-VL-7B     |     35.2 |   44.4 |    27.6 |    97.6 |      51.2 |
| Qwen2.5-VL-72B    |     38.8 |   36   |    33.2 |    96.4 |      51.1 |
| Qwen3-VL-8B       |     36.8 |   39.2 |    28.8 |    99.6 |      51.1 |
| GLM-4.6V          |     39.2 |   35.6 |    25.6 |   100   |      50.1 |
| Gemini-3-Pro      |     40.8 |   41.2 |    32   |    94   |      52   |
| Qwen2.5-VL-32B    |     36   |   34.4 |    30.4 |    89.2 |      47.5 |
| Pixtral-12B       |     26   |   27.2 |    28.4 |    24.8 |      26.6 |
| GLM-4.5V          |     25.2 |   22   |    22.8 |    26.8 |      24.2 |
...
## E Cross-Benchmark Validation

## E.1 Full FRB Cross-Benchmark Results
...
## E.2 Stroop Effect Visualization
...
Table 11. Cross-benchmark evaluation on the FRB 15-font MCQ format [Li et al., 2025]. Easy: normal sentences; Hard: stroop condition where the text spells a font name. Random baseline is 6.7%.

| Model                                  |   FRB Easy |   FRB Hard |   FRB Overall |
|----------------------------------------|------------|------------|---------------|
| Open-Source Models Qwen2.5-VL-7B       |        7.3 |        6.7 |           6.9 |
| Qwen2.5-VL-32B                         |        8.7 |        6.7 |           7.5 |
| Qwen2.5-VL-72B                         |        9.3 |        6.7 |           7.7 |
| Qwen3-VL-8B                            |       14   |        6.7 |           9.6 |
| Qwen3-VL-30B-A3B                       |       14.7 |        6.7 |           9.9 |
| Qwen3-VL-32B                           |       14   |        6.7 |           9.6 |
| Pixtral-12B                            |        9.3 |        8.4 |           8.8 |
| GLM-4.5V                               |        2.7 |        7.1 |           5.3 |
| GLM-4.6V                               |        9.3 |        6.7 |           7.7 |
| Closed-Source Models Qwen3-Max         |       15.3 |        6.7 |          10.1 |
| GPT-5.2                                |       26.7 |       16.9 |          20.8 |
| Gemini-3-Flash                         |       56.7 |       29.8 |          40.5 |
| Gemini-3-Pro                           |       24.7 |       10.7 |          16.3 |
| Claude-Sonnet-4.6                      |       34   |       15.6 |          22.9 |
| Doubao-Seed-1.6                        |       10.7 |        6.7 |           8.3 |
| Fine-Tuned Models Qwen2.5-VL-7B + LoRA |       14   |        6.7 |           9.6 |
...
Figure 10. The stroop effect [Li et al., 2025] evaluated across all models. Under normal text, Gemini-3-Flash achieves 56.7%, but when text spells a conflicting font name, most models collapse to the 6.7% random baseline. Claude-Sonnet-4.6 drops from 34.0% to 15.6%.

<!-- image -->

## F Detailed Analysis

## F.1 Per-Property Detailed Analysis

## F.1.1 Font Family

Font family recognition varies widely across models, with Gemini-3-Flash achieving 80.8% while most open-source models hover around 35-50%. The difficulty stems from subtle visual differences between fonts; distinguishing Arial from Helvetica requires perceiving minute variations in character proportions and stroke terminals. Three failure patterns dominate. Within-category confusion : models frequently mistake one sans-serif font for another but rarely confuse a serif font with a monospace one. Default-response bias : some models overwhelmingly predict common fonts

like Arial or Times New Roman regardless of actual content. OCR interference : models appear to focus on reading the text rather than perceiving its visual presentation.

## F.1.2 Font Size

Table 12 shows performance by size bucket. Models exhibit strikingly different size perception strategies. Qwen3VL-8B and Gemini-3-Flash both achieve 0% on XLarge sizes (48-64pt), systematically predicting 'large' instead, suggesting these models lack the concept of extra-large font sizes. In contrast, GPT-5.2 achieves 90.3% on XLarge and 76.3% on Small, demonstrating accurate perception at size extremes but struggling with intermediate sizes at only 35.5% on Medium.

Table 12. Font size recognition accuracy (%) by size bucket. Table 13. Style confusion matrix for Qwen3-VL-8B (%).

| Model          | S    | M    | L    | XL   | True/Pred   |   Reg |   Bld |   Ita |   B-I |
|----------------|------|------|------|------|-------------|-------|-------|-------|-------|
| Qwen3-VL-8B    | 18.6 | 58.9 | 45.3 | 0.0  | Regular     |  69.2 |  14.1 |  16.7 |   0   |
| Gemini-3-Flash | 57.6 | 46.7 | 88.7 | 0.0  | Bold        |  70.5 |  13.7 |  15.8 |   0   |
| GPT-5.2        | 76.3 | 35.5 | 45.3 | 90.3 | Italic      |  80   |   5   |  12.5 |   2.5 |
|                |      |      |      |      | Bold-Italic |  67.6 |  10.8 |  21.6 |   0   |
...
Table 14. Color recognition accuracy (%) by color. Most models achieve near-perfect accuracy; GLM-4.5V fails across all colors.
...
## F.2 Scaling Paradox Details

Table 15. Model size vs. overall accuracy. Larger models do not consistently outperform smaller ones.
...
In the Qwen2.5 family, the scaling curve is non-monotonic: the 7B model at 51.2% outperforms the 32B model at 47.5% by 3.7 pp, while the 72B model recovers only to 51.1%, nearly matching the 7B. The 32B model shows notably degraded color recognition (89.2% vs 97.6% for the 7B) and worse font size accuracy (34.4% vs 44.4%), suggesting

that intermediate scales can introduce regressions in fine-grained visual perception. In the Qwen3 family, the 30B-A3B MoE variant at 54.4% outperforms both the 8B at 51.1% and the 32B dense model at 51.4%, with improvements concentrated in font family recognition (49.6% vs 36.8% for the 8B).
...
Within-category confusion. Models frequently confuse fonts within the same category: Arial, Helvetica, and Verdana are mutually confused among sans-serif fonts; Times New Roman, Georgia, and Palatino among serif fonts; and Helvetica and Helvetica Neue represent the hardest variant-level confusion.

Default-response collapse. Pixtral-12B (26.6% overall, 24.8% color) and GLM-4.5V (24.2% overall, 26.8% color) perform near the 25% random baseline across all four properties including color, indicating that these models do not engage with the image at all for typographic queries.

Size estimation errors. Adjacent sizes are the most common confusion (e.g., 24 pt vs. 32 pt). Some models show near-zero accuracy on the XLarge bucket (48-64 pt), suggesting they never output the extreme size labels. Middle-range sizes (20-40 pt) are the hardest for most models due to overlapping relative appearance.

## G Discussion

## G.1 Limitations

Benchmark scale. FontBench comprises 250 samples yielding 1,000 questions. While sufficient to detect large performance differences (95% confidence intervals span ± 3pp), this scale limits statistical power for fine-grained comparisons. Non-Latin coverage is particularly sparse: CJK has only 18 samples and Arabic only 11.
...
Example:
```markdown
![Description based on OCR](../media/doc_1773473585_d49d14d0_arxiv_2603.08497.pdf) <!-- TODO: Broken image path -->
