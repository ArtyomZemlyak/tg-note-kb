...
### Архитектура: Gated Softmax Attention / Gated Attention

**Проблема:** Нестабильность больших трансформеров и проблема attention sinks (нестабильности, связанные с чрезмерным вниманием к отдельным токенам), а также phenomenon Massive Activations (чрезмерно большие значения в скрытых состояниях). Эти проблемы особенно актуальны для Mixture-of-Experts (MoE) архитектур, склонных к спайкам лосса.

**Решение:** Добавление обучаемого зависимого от входа сигмоидного гейта сразу после выхода Scaled Dot-Product Attention (SDPA). Модулируя выход Y гейтом σ(XW_θ), метод вносит поэлементную разреженность и нелинейность перед финальной проекцией. Это позволяет моделям "отвергать" неполезный выход внимания, если он неинформативен, вместо того чтобы полагаться на нормализацию Softmax.

**Применение:** Уже используется в Qwen3-Next как часть гибридной архитектуры с Gated DeltaNet в соотношении 3:1.

**Преимущества:**
- Повышенная стабильность при обучении больших моделей (1.7B dense и 15B MoE) 
- Радикальное устранение phenomenon Attention Sink (~46.7% внимания к первому токену снижается до ~4.8%)
- Устранение Massive Activations и предотвращение их распространения через residual stream
- Минимальные вычислительные издержки (менее 2% wall-time)
- Улучшенная масштабируемость архитектур
- Стабильно улучшенная перплексия на различных размерах моделей
- Значительно улучшенная экстраполяция на длинный контекст

### Обучение с подкреплением: Масштабирование до 1024 слоёв

**Self-Supervised RL** — позволил обучать сеть глубиной 1024 слоя, что ранее было невозможно для стандартных подходов. На задачах локомоции достигнуто улучшение до 50× по сравнению с обычными моделями. Работа "1000 Layer Networks for Self-Supervised RL: Scaling Depth Can Enable New Goal-Reaching Capabilities" (авторы Kevin Wang, Ishaan Javali, Michał Bortkiewicz, Tomasz Trzciński, Benjamin Eysenbach) представляет контрастное обучение с подкреплением (Contrastive RL) в сочетании с современной архитектурной "начинкой": резидуальные связи, нормализация по слою и активации Swish.

**Ключевые инновации:**
- Контрастное обучение вместо традиционной регрессии (InfoNCE loss)
- Современные архитектурные элементы: Residual Blocks, LayerNorm, Swish Activation
- Self-supervised формализм для goal-conditioned RL

**Преимущества:**
- Возможность обучения экстремально глубоких сетей
- Значительное улучшение производительности
- Применение к задачам, где глубокие архитектуры особенно полезны
- Эмерджентные навыки локомоции без сложного инжиниринга наград
...
### Perception Encoder: Открытие промежуточных слоев

Одной из ключевых статей конференции стала работа "Perception Encoder: The best visual embeddings are not at the output of the network", в которой исследователи от Meta AI представили новую модель, где лучшие визуальные эмбеддинги извлекаются не из последнего, а из промежуточных слоёв трансформера. Исследование показало, что традиционный подход извлечения эмбеддингов из последнего слоя не оптимален для downstream-задач, и что промежуточные слои содержат более полезную информацию.
