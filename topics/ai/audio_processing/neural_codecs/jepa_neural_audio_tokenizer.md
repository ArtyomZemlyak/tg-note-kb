...
Как только энкодер предобучен, его веса замораживаются. К нему пристыковываются квантователь и декодер. Вместо векторного квантования (VQ) с кодовыми книгами, авторы используют [Finite Scalar Quantization (FSQ)](fsq_neural_quantization.md).

Непрерывные латентные вектора проецируются через tanh в диапазон (-1, 1), а затем квантуются в дискретные уровни L = [4, 4, 4, 4] на измерение. Эти индексы упаковываются через смешанную систему счисления. Для группы из G=7 измерений итоговый токен считается методом Горнера: token = sum(i_k * prod(r_j)). Это дает словарь 4^7 ~ 16k (как в NLP) и пропускную способность 47.5 токенов/сек. Восстанавливает аудио декодер на базе [HiFi-GAN](hifi_gan_decoder.md).
...
![Stage 1: JEPA Pretraining Architecture](../../../../media/img_1766039991_aqadkhnrg5clgup_stage_1_jepa_pretraining_image.jpg)
*Рисунок: Архитектура предобучения JEPA, показывающая двухэтапный процесс обучения с Context и Target энкодерами*

![JEPA Online Encoder Architecture](../../../../media/img_1766039991_aqadkxnrg5clgup_figure_2_jepa_online_encoder_architectur.jpg)
*Рисунок: Архитектура онлайн-энкодера JEPA с DAAM-механизмом*

![JEPA Predictor Network Architecture](../../../../media/img_1766039991_aqadlbnrg5clgup_figure_3_jepa_predictor_network_architec.jpg)
*Рисунок: Архитектура предикторной сети JEPA*

![Stage 1 JEPA Masked Architecture](../../../../media/img_1766039991_aqadlxnrg5clgup_figure_4_stage_1_jepa_masked.jpg)
*Рисунок: Пример маскирования в Stage 1 JEPA*

![HiFi-GAN Decoder Architecture](../../../../media/img_1766039991_aqadlrnrg5clgup_figure_5_hifi_gan_decoder_architecture_s.jpg)
*Рисунок: Архитектура HiFi-GAN декодера*

![Frame Rate Comparison with Existing Models](../../../../media/img_1766039991_aqadmbnrg5clgup_lable_2_kkrame_rate_comparison_with.jpg)
*Рисунок: Сравнение частоты кадров с существующими моделями (JEPA работает на частоте 2.5 Гц)*

![Model Architecture and Parameters](../../../../media/img_1766039991_aqadlhnrg5clgup9_table_3_model_architecture_and_parameter.jpg)
