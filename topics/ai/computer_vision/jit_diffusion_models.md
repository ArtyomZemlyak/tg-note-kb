...
| Аспект | JiT в пиксельном пространстве | PixelDiT | Латентные диффузионные модели |
|--------|-------------------------------|----------|-------------------------------|
| Пространство работы | Пиксельное | Пиксельное | Латентное (через автоэнкодер) |
| Вычислительная сложность | Выше из-за высокой размерности | Умеренная (с использованием двухуровневого каскада) | Ниже за счет сжатия |
| Качество генерации | Приемлемое, но не SOTA | Высокое (с использованием пиксель-уровневой модуляции) | Обычно лучше |
| Архитектурная сложность | Проще (без автоэнкодера) | Более сложная (двухуровневый каскад) | Сложнее (включает энкодер/декодер) |
