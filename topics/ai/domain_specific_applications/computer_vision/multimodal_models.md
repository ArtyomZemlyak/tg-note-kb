...
### VL-JEPA
- Визуально-языковая модель с Joint Embedding Predictive Architecture (JEPA)
- Предсказывает непрерывные текстовые эмбеддинги вместо дискретных токенов
- Разрывает семантическое рассуждение и синтаксическую генерацию
- Использует селективное декодирование для повышения эффективности
- Относится к семейству JEPA архитектур, вдохновленных концепцией World Model Яна Лекуна
