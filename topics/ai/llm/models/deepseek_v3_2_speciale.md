...
DeepSeek применяют несколько методов стабилизации обучения с подкреплением:

1. **Unbiased KL Estimate** - улучшение GRPO, которое устраняет систематическую ошибку в оценке дивергенции Кульбака-Лейблера. Исправление заключается в перевзвешивании KL-члена с тем же самым коэффициентом важности (importance ratio), что и используется для основной функции потерь. Это делает градиент KL-ошибки несмещенным, предотвращая градиентный взрыв, шумные обновления и нестабильную динамику обучения.

2. **Off-Policy Sequence Masking** - маскируют последовательности, которые отклоняются от исходной модели больше определенного порога, если они хуже средней в группе генераций по оценке.

3. **Сохранение масок токенов** - сохраняют маски токенов, полученные из top-p / top-k семплирования, чтобы выровнять режимы генерации и обучения.
...
- [[unbiased_kl_estimate_in_grpo.md]] - улучшение GRPO с несмещенной оценкой KL-дивергенции, используемое в V3.2
...
- [[deepseek_v3_2_key_innovations.md]] - обзор всех ключевых инноваций V3.2
