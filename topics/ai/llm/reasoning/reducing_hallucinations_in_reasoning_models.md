...
Результаты, представленные в статье (часть II), показывают значительное улучшение фактологичности модели:
...
Также было показано, что SFT + DPO эксперимент сильно снижает полезность ответа при примерно таком же качестве детальности (Dtl) и фактической точности (Pre) по сравнению с SFT + GRPO, что делает предложенный подход (SFT + online GRPO) более актуальным.
