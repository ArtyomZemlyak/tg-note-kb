...
4. **Параметрически эффективное обучение**: Методы, при которых обновляется только небольшая часть параметров модели, например, через адаптеры, LoRA (Low-Rank Adaptation) или векторно-управляемые подсказки.
...
6. **Оптимизаторы с эффективным использованием памяти**: Специализированные оптимизаторы, такие как [[algorithms/classical_ml/optimization/algorithms/flashoptim_optimizer.md|FlashOptim]], которые уменьшают потребление памяти на параметр более чем на 50% через квантование состояний и разделение весов.
...
- [[matrix_whitening_optimizers.md]] - Оптимизаторы с матричным отбеливанием, которые используют низкоранговые факторизации для снижения потребления памяти
- [[../llm/reasoning/vector_guided_reasoning.md]] - Пример векторно-управляемого подхода к параметрически эффективному обучению, применительно к задаче развития способностей к логическим рассуждениям
- [[algorithms/classical_ml/optimization/algorithms/flashoptim_optimizer.md]] - FlashOptim: оптимизаторы с квантованием состояний и разделение весов для экономии >50% памяти
- [[algorithms/classical_ml/optimization/algorithms/muon_optimizer.md]] - Muon: оптимизатор с ортогонализацией Ньютона-Шульца для эффективного обучения
