...
## Проблемы низкоточного обучения с Flash Attention

### Катастрофический взрыв потерь в BF16

Использование Flash Attention в низкоточных форматах (BF16) может приводить к **катастрофическому взрыву потерь** (loss explosion) после нескольких тысяч шагов обучения. Эта проблема документировалась в сообществе более 2 лет (nanoGPT Issue #303, #524, #554) без понимания коренной причины.

Исследование "Why Low-Precision Transformer Training Fails: An Analysis on Flash Attention" (Qiu & Yao, Tsinghua University, 2025) впервые предоставило механистическое объяснение этой проблемы.

### Коренные причины нестабильности

Проблема вызвана взаимодействием **двух взаимосвязанных феноменов**:

1. **Возникновение схожих низкоранговых представлений** в механизме внимания across токенов и шагов обучения
2. **Накопление систематических ошибок округления** в арифметике BF16 (7 бит мантиссы)

Эти факторы создают **порочный круг накопления ошибок**, который искажает обновления весов и разрушает динамику обучения.

### Механизм сбоя

- Ошибка происходит в вычислении **δ-термина** в backward pass: `δ = rowsum(dO ∘ O)`
- В BF16 вычисление выхода `O` содержит систематическую ошибку округления
- Ошибка округления **положительно смещена** из-за специфического распределения данных в P̄V произведении
- Это приводит к накоплению низкоранговой ошибки в весах, увеличению спектральной нормы и активаций, и最終 к взрыву потерь

### Локализация проблемы

- Проблема originates в **одном слое** (обычно слой 2 в GPT-2)
- Проблема локализована в **конкретных головах внимания** (головы 1, 7, 8, 9, 11, 12 в GPT-2)
- **Голова 8** обычно имеет наибольшую спектральную норму

### Решения и смягчение

**Немедленные решения**:
- Вычисление выхода `O` в **FP32** во время forward pass
- Использование альтернативной формулировки: `δ = rowsum(dP ∘ P)` вместо `δ = rowsum(dO ∘ O)`
- Селективное использование FP32 только для проблемных голов внимания

**Долгосрочные решения**:
- Минимальная модификация Flash Attention для смягчения ошибок округления (предложена в исследовании)
- Гибридная точность: BF16 для memory-bound операций, FP32 для критических вычислений

**См. также**: [[optimization/low_precision_training_instability.md]] - Подробный анализ проблем нестабильности низкоточного обучения
