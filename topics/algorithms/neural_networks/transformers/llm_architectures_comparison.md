...
- **Недавние исследования (2025)**: показали, что энкодер-декодер архитектуры могут превосходить декодер-только по эффективности инференса и экстраполяции на длинные контексты
- **Преимущества после файнтюнинга**: как показано в исследованиях с RedLLM, энкодер-декодер архитектуры демонстрируют удивительную адаптивность после instruction-файнтюнинга, достигая сравнимых или даже лучших результатов, чем декодер-только модели
- **Эффективность инференса**: доминируют на Парето-фронте "качество/затраты на инференс"
- **Стратегия внимания**: cross-attention не подвержен "затуханию локальности", что важно для длинных последовательностей

## Сравнение эффективности архитектур

### Предобучение vs Файнтюнинг

Недавние исследования (Encoder-Decoder or Decoder-Only? Revisiting Encoder-Decoder Large Language Model, 2025) выявили интересную двухэтапную картину:

#### Этап 1: Предобучение
- **Decoder-only модели** (DecLLM) показывают лучший вычислительно-оптимальный скейлинг
- Достигают меньшей перплексии при заданном объеме вычислений
- Показывают более сильные навыки в zero-shot и few-shot режимах до файнтюнинга

#### Этап 2: После instruction-файнтюнинга
- **Encoder-decoder модели** (RedLLM) демонстрируют удивительную адаптивность
- После файнтюнинга на датасете FLAN, RedLLM не только сокращает разрыв, но и достигает сравнимых и даже немного лучших результатов
- При 8B параметров few-shot результат RedLLM составляет 61.32, превосходя 59.02 у DecLLM

### Эффективность инференса

#### Encoder-decoder архитектура:
- **Высокая пропускная способность**: значительно более высокая пропускная способность как при обучении, так и при инференсе
- **Парето-доминирование**: по осям качество/затраты на инференс (FLOPs) доминирует на Парето-фронте
- **Архитектурные преимущества**: cross-attention в RedLLM не подвержен "затуханию локальности", обеспечивает постоянный поток информации из промпта

#### Decoder-only архитектура:
- **Традиционная эффективность**: долгое время считалась более эффективной, особенно при предобучении
- **KV-кеширование**: эффективно используется на этапе prefilling и генерации
- **Ограничения при длинных контекстах**: подвержена "затуханию локальности" - токены уделяют меньше внимания удалённым токенам по мере удлинения последовательности

### Экстраполяция на длинные контексты

- **Encoder-decoder**: перплексия растет гораздо плавнее при обработке последовательностей длиннее, чем контекст предобучения
- **Decoder-only**: производительность ухудшается более резко на длинных последовательностях
...
- **ZAYA1**: Первая MoE модель, полностью обученная на стеке AMD (Instinct GPU, ROCm), с 8.3 млрд. параметров и 800 млн. активных параметров, использующая инновационный Compressed Convolutional Attention механизм внимания
...
- Новый подход к реализации разреженного внимания, который использует обучаемую, основанную на содержании маршрутизацию, вдохновлённую концепцией Смесь Экспертов (Mixture of Experts, MoE)
- В отличие от традиционных методов разреженного внимания с фиксированными шаблонами, MoSA позволяет каждому головному элементу внимания динамически выбирать разреженное подмножество токенов на основе содержания ввода
- Является первым методом разреженного внимания, который последовательно превосходит традиционные модели с полным вниманием по производительности
- Снижает вычислительные затраты и требования к памяти без потери качества модели

### Star Attention
- Новая архитектура разреженного блочного внимания, разработанная NVIDIA для эффективного вывода в LLM на длинных последовательностях
- Использует двухфазный подход: кодирование контекста и кодирование запроса с генерацией токенов
- Контекст разбивается на соприкасающиеся блоки, каждый из которых (кроме первого) начинается с якорного блока для решения проблемы attention sinks
- Обеспечивает значительное ускорение (до 11x) при сохранении 97-100% точности по сравнению с глобальным вниманием
- Совместим с существующими моделями без необходимости переобучения
- Требует минимальной коммуникации между хостами за счет эффективного распределенного вычисления глобального softmax
...
- [[../../../ai/llm/architectures/llm_architecture_gallery.md|Галерея архитектур LLM]] — визуальный обзор и классификация различных архитектур LLM
