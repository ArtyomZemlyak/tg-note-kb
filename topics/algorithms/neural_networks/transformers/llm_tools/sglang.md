...
1. **Производительность**:
   ...
   - В недавнем анализе оптимизации FP4 кернелов для MoE на NVIDIA Blackwell достигает 1262 TFLOPS при размере батча 4096, что выше, чем FlashInfer (1225 TFLOPS) и vLLM (1117 TFLOPS)
   - Особенно хорош при малых батчах (206.9μs/слой при BS=1), обеспечивая 1.32x ускорение над vLLM
...
2. **Архитектура**:
   ...
   - Агрессивное слияние кернелов: Сокращает 7 запусков кернелов до 5 для MoE, устраняя накладные расходы на перестановку
   - Адаптивное масштабирование сетки: Динамически максимизирует занятость SM при малых батчах
...
4. **Поддержка оборудования**:
   ...
   - Специфические оптимизации для NVIDIA Blackwell: Использует специфичные расписания CUTLASS для Blackwell, такие как `KernelPtrArrayTmaWarpSpecialized1SmNvf4Sm100` для работы с FP4
...
- [[nvfp4_format.md]] - NVFP4 формат квантования, используемый в оптимизациях SGLang для Blackwell
- [[../../../../../tools_and_platforms/hardware/nvidia_blackwell_architecture.md|Архитектура NVIDIA Blackwell]] - Архитектура GPU, для которой SGLang реализует специфичные оптимизации
- [[blackwell_fp4_moe_optimization.md]] - Подробный анализ производительности SGLang в сравнении с другими фреймворками на Blackwell
