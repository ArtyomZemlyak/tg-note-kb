...
### Гиперсети для генерации LoRA

Современные исследования предлагают использовать гиперсети для мгновенной генерации LoRA-адаптеров:

- **Doc-to-LoRA** [[../../../ai/llm/doc_to_lora.md]] — генерация LoRA из документов для интернализации фактологических знаний без переобучения. Решает проблему долгосрочной памяти в LLM.
- **Text-to-LoRA** [[../../../ai/llm/text_to_lora.md]] — генерация LoRA из описаний задач для мгновенной адаптации к новым задачам. Заменяет дорогие пайплайны тонкой настройки.
- **Tencent HY-WU** [[../../../ai/neural_architectures/hypernetworks/tencent_hy_wu_weight_unleashing.md]] — динамическая генерация LoRA для редактирования изображений. Модель-генератор (8B) синтезирует LoRA (~0.72B) под каждый пример во время инференса, решая проблему конфликта градиентов между разнородными задачами.

Все методы используют **двухфазный подход**: дорогое мета-обучение один раз + дешевая генерация адаптеров при развертывании.
...
- [[ai/llm/doc_to_lora.md]] - Doc-to-LoRA: гиперсети для генерации LoRA из документов (долгосрочная память)
- [[ai/llm/text_to_lora.md]] - Text-to-LoRA: гиперсети для генерации LoRA из описаний задач (адаптация)
- [[ai/llm/tinylora_extremely_parameter_efficient_finetuning.md]] - TinyLoRA: экстремальное сокращение параметров LoRA до минимума
