...
- **Эффективность квантования**: Возможность эффективного использования форматов низкой точности, таких как NVFP4, для дополнительной оптимизации
...
- **Qwen3.5-Plus**: Предельно разреженная архитектура — 397B общих параметров, только 17B активных (~4.3%), менее 5% вычислительной мощности используется на запрос

### Qwen3.5-Plus MoE реализация

Модель **Qwen3.5-Plus** (выпущена в канун Лунного Нового года 2026) демонстрирует революционный подход к MoE:
- **397 миллиардов** общих параметров
- **17 миллиардов** активных параметров во время генерации
- **Менее 5%** вычислительной мощности на запрос
- Обгоняет по качеству предшественницу с триллионом параметров
- Снижение стоимости развёртывания на 60%
- Увеличение скорости обработки запросов до 19 раз

## Оптимизации инференса

### Оптимизации для современных GPU архитектур

MoE модели требуют специфических оптимизаций для эффективной работы на современных GPU, особенно на архитектуре NVIDIA Blackwell:

- **Оптимизация FP4 кернелов**: Интеграция с NVFP4 форматом для эффективного квантования и вычислений
- **Слияние кернелов**: Сокращение количества CUDA кернелов с 7 до 5 в SGLang для уменьшения накладных расходов
- **Адаптивное масштабирование**: Динамическая настройка конфигурации запуска кернелов для максимизации загрузки SM
- **Expert-first layout**: Преобразование данных из токен-первый в эксперт-первый формат для лучшего использования памяти
- **Перекрытие IO и вычислений**: Архитектурные возможности Blackwell позволяют перекрывать операции ввода-вывода с вычислениями
...
- [[qwen3_5_plus.md|Qwen3.5-Plus]] - Предельно разреженная MoE архитектура с 397B параметрами (17B активными)
...
- [[inference/blackwell_fp4_moe_optimization.md|Оптимизация FP4 кернелов для MoE на NVIDIA Blackwell]] - Сравнительный анализ оптимизации MoE в различных фреймворках
- [[inference/vllm_summary.md|vLLM]] - Фреймворк для эффективного инференса, включая MoE модели
- [[tools/sglang.md|SGLang]] - Фреймворк с высокой производительностью для инференса MoE моделей
- [[tools/flashinfer.md|FlashInfer]] - Библиотека для оптимизации инференса MoE моделей

## Источники

1. Различные научные публикации о MoE архитектурах
2. Документация по моделям DeepSeek, Llama, Qwen
3. **Telegram-канал «Китайский ИИ»**, пост о выпуске Qwen3.5-Plus, Лунный Новый год 2026 — информация о MoE реализации в Qwen3.5-Plus
4. [Alibaba Qwen Official (@Alibaba_Qwen) / X (Twitter)](https://x.com/Alibaba_Qwen) - официальная информация о Qwen3.5-Plus