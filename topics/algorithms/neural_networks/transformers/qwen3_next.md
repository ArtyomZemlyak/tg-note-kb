...
- **Gated Attention**: Механизм, добавляющий обучаемый зависимый от входа сигмоидный гейт сразу после выхода масштабированного скалярного произведения внимания (SDPA). Модулируя выход Y гейтом σ(XW_θ), метод вносит поэлементную разреженность и нелинейность перед финальной проекцией. Это решает ключевые проблемы трансформеров: линейность между проекциями, Attention Sink и Massive Activations.
- **Архитектурные особенности**: Включает RMSNorm с нулевым центром для QK-norm и частичным RoPE
- **Распределение**: Блоки расположены в соотношении 3:1 (3 блока Gated DeltaNet на 1 блок Gated Attention)
- **Преимущества**: Решает проблемы стабильности обучения, устраняет Attention Sink (~46.7% внимания к первому токену снижается до ~4.8%), улучшает экстраполяцию на длинный контекст
...
- [[../architectures/gated_deltanet.md|Gated DeltaNet]] - Подробное объяснение новой архитектуры внимания
