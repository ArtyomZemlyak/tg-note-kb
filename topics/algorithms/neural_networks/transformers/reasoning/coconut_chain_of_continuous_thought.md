...
![Coconut архитектура](../../../../images/img_1763206945_AgACAgIA.jpg) <!-- TODO: Broken image path -->
...
Недавнее исследование "Do Latent Tokens Think? A Causal and Adversarial Analysis of Chain-of-Continuous-Thought" выявило серьезные проблемы с подходом COCONUT. Исследование показало, что:

- Латентные токены COCONUT функционируют как непонятные заглушки, а не как faithful кодирование рассуждений
- Метод уязвим к обучению на шорткатах, эксплуатируя артефакты датасета вместо подлинного логического мышления
- COCONUT показывает минимальную чувствительность к каузальным пертурбациям (успех менее 10%), в то время как традиционный CoT показывает высокую чувствительность (до 50% на AdvBench)
- Замена латентных токенов между разными входами практически не влияет на точность COCONUT (~60% сохраняется), в то время как замена явных токенов CoT снижает метрики до ~43%

Эти результаты ставят под сомнение утверждения о том, что COCONUT обеспечивает подлинное рассуждение, и указывают на то, что улучшенная производительность на бенчмарках может быть иллюзорной.

## Сравнение с критикой

В отличие от утверждений о преимуществах COCONUT, новое исследование показывает, что:
- COCONUT использует "псевдо-рассуждение", которое маскирует зависимость от шорткатов
- Латентные токены не обладают каузальной силой в процессе принятия решений
- Метод более склонен к эксплуатации артефактов датасета, чем к истинному логическому мышлению
...
- [[trm_architecture.md]] - архитектура Tiny Recursive Model с альтернативным подходом к рассуждению
- [[computational_graphs_in_llm_reasoning.md]] - вычислительные графы в рассуждениях LLM
- [[coconut_critical_analysis.md]] - подробный критический анализ COCONUT
...
3. [Do Latent Tokens Think? A Causal and Adversarial Analysis of Chain-of-Continuous-Thought](https://arxiv.org/abs/2512.21711) - критический анализ COCONUT, показывающий его уязвимости и склонность к использованию шорткатов