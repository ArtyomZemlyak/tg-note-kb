...
![Сравнение Coconut с другими подходами к рассуждению](../../../../images/img_1763208338_AgACAgIA.jpg) <!-- TODO: Broken image path -->
...
## Критическая оценка COCONUT

Недавнее исследование "Do Latent Tokens Think? A Causal and Adversarial Analysis of Chain-of-Continuous-Thought" показало существенные проблемы с Coconut:

### Результаты критического анализа:
- Coconut показывает минимальную чувствительность к каузальным пертурбациям (менее 10% успеха), в то время как CoT показывает высокую чувствительность (до 50% на AdvBench)
- При замене латентных токенов между разными входами точность Coconut почти не падает (~60%), в то время как при замене явных токенов CoT точность падает до ~43%
- Coconut склонен к использованию шорткатов и эксплуатации артефактов датасета, а не к подлинному логическому рассуждению
- Латентные токены Coconut функционируют как непонятные заглушки, а не как кодирование подлинного мышления

### Пересмотренные экспериментальные результаты:
С учетом новых находок:
- Улучшенная точность на бенчмарках может быть результатом эксплуатации шорткатов, а не подлинного рассуждения
- Coconut стабильно превосходит LLM без CoT, но это может быть связано с адаптацией к артефактам датасета
- На математических задачах (GSM8k) Coconut может показывать худшие результаты по сравнению с тщательно настроенным CoT
- Coconut использует меньше токенов, но это преимущество достигается за счет качества рассуждений

## Сравнение с учетом критики

### В свете критического анализа:
- **Coconut vs CoT**: Хотя Coconut может показывать лучшую точность на бенчмарках, он менее надежен и более подвержен шорткатам
- **Coconut vs ToT**: Coconut менее контролируемый и менее интерпретируемый, чем ToT, с дополнительной уязвимостью к псевдо-рассуждениям
- **Coconut vs TRM**: TRM может быть более стабильной альтернативой, так как его рекурсивная архитектура обеспечивает более прозрачный процесс рассуждения

## Связанные темы

- [[coconut_chain_of_continuous_thought.md]] - основная информация о Coconut
- [[coconut_critical_analysis.md]] - критический анализ Coconut
...
5. [Do Latent Tokens Think? A Causal and Adversarial Analysis of Chain-of-Continuous-Thought](https://arxiv.org/abs/2512.21711) - критический анализ COCONUT, выявляющий его уязвимости и склонность к использованию шорткатов