...
### Технические детали
- Использует свертки внутри блока внимания
- Основан на интеграции сверточных операций в стандартную архитектуру внимания
- Обеспечивает компрессию информации через свертки
- Уменьшает объем вычислений, необходимых для вычисления матрицы внимания

### Преимущества
- **Снижение вычислительной сложности**: по сравнению с классическим Multi-Head Attention
- **Уменьшенное потребление памяти**: за счет более эффективного представления контекста  
- **Сохранение качества**: способность модели к качественной обработке сложных задач
- **Эффективность для MoE-архитектур**: особенно полезен в моделях с разреженными вычислениями

### Применение
- **ZAYA1**: первая модель, использовавшая этот механизм, достигнув высоких результатов в задачах резонинга, математики и программирования

## Gated Attention

### Описание
Gated Attention - это архитектурное изменение стандартного механизма внимания в трансформерах, призванное решить проблемы линейности, Attention Sink и Massive Activations. Механизм добавляет обучаемый зависимый от входа сигмоидный гейт сразу после выхода масштабированного скалярного произведения внимания (SDPA). Модулируя выход Y гейтом σ(XW_θ), метод вносит поэлементную разреженность и нелинейность перед финальной проекцией.

### Проблемы, решаемые Gated Attention
- **Проблема линейности**: Устраняет ограничения, накладываемые линейной композицией проекций Value (W_V) и Output (W_O)
- **Attention Sink**: Механистически устраняет феномен, при котором модель сливает "лишнюю" массу внимания на первый токен из-за нормализации Softmax
- **Massive Activations**: Устраняет накопление огромных значений в скрытых состояниях, предотвращая их распространение через residual stream
- **Нестабильность обучения**: Особенно в Mixture-of-Experts (MoE) архитектурах, которые склонны к спайкам лосса

### Технические детали
- **Модификация**: Добавление гейта после SDPA: Y' = Y ⊙ σ(X W_θ)
- **Где применяется**: Поэлементный гейтинг выхода SDPA для каждой головы
- **Нелинейность**: Введение мультипликативного взаимодействия, которого нет в ванильных трансформерах
- **Зависимость от входа**: Гейт работает от скрытых состояний X (вход в слой после нормализации)

### Преимущества
- **Стабильность обучения**: Значительное улучшение ландшафта оптимизации
- **Разреженность**: Введение зависимой от входа разреженности
- **Минимальный оверхед**: Менее 2% wall-time
- **Улучшенная масштабируемость**: Лучшие результаты на 1.7B dense и 15B MoE моделях
- **Устранение Attention Sink**: Снижение внимания к первому токену с ~46.7% до ~4.8%
- **Лучшая экстраполяция**: Улучшенная работа с длинными контекстами

### Применение
- **Qwen3-Next**: Использует гибрид Gated DeltaNet и Gated Attention в соотношении 3:1
- **MoE архитектуры**: Улучшение стабильности обучения
- **Модели с длинным контекстом**: Решение проблемы Attention Sink

### Экспериментальные результаты
![Investigated positions for applying gating operations](../../../../media/img_1764342547_aqadza1rgr9sul_figure_left_investigated_positions_for.jpg) <!-- TODO: Broken image path -->

**Описание:** График показывает различные позиции для применения гейтинг-операций внутри слоя self-attention. Видно, что гейтинг после SDPA (G1) дает наилучшие результаты, а также сравнение потерь при обучении между базовой и SDPA-гейтированной 1.7B dense моделями, демонстрирующее улучшенную стабильность обучения с гейтингом.
...
| Gated Attention | O(n²) | O(n²) | Высокое | Механизм, решающий проблемы стабильности больших трансформеров, Attention Sink и Massive Activations, добавляя обучаемый гейт после SDPA |
| Gated Softmax Attention | O(n²) | O(n²) | Высокое | Механизм, решающий проблемы стабильности больших трансформеров и attention sinks, добавляющий sigmoid-гейтирование к отдельным головам (описан в результатах NeurIPS 2025) |
...
- **Star Attention**: Используется для эффективного инференса в LLM с длинными контекстами, может быть интegrated в существующие модели без переобучения
...
- **Qwen3-Next**: Использует гибрид Gated DeltaNet и Gated Attention в соотношении 3:1, также включает Gated Softmax Attention (описанный в результатах NeurIPS 2025)
...
### Технические детали
- **Пошаговый отбор**: Использует механизм внимания для построения подмножеств пошагово
- **Контекстное обучение**: Сохраняет набор выбранных кандидатов и использует их как контекст для поиска следующего самого информативного кандидата
- **Жадный отбор**: Позволяет модели сделать локально оптимальные решения о том, какой элемент включить на каждом шаге
- **Оценка важности**: Использует "внимательные баллы" для количественной оценки важности кандидатов на основе текущих выбранных кандидатов
- **Последовательная адаптация**: В отличие от стандартного внимания, которое взвешивает всех кандидатов одновременно, Sequential Attention работает последовательно, адаптируясь к предыдущим выборам

### Преимущества
- **Эффективность и точность**: Позволяет параллельно обрабатывать кандидаты после расчёта баллов внимания, оценивая их быстрее, чем традиционные последовательные методы отбора
- **Интерпретируемость**: Баллы внимания служат диагностическими инструментами, показывающими, на какие части входных данных модель обращала приоритетное внимание при принятии решений
- **Масштабируемость**: Эффективно обрабатывает большое количество кандидатов, что критично для последовательного отбора функций в современных нейронных сетях
- **Минимальные затраты**: Интегрирует выбор непосредственно в процесс обучения модели, избегая увеличения затрат на обучение в разы

### Применение
- **Оптимизация производительности языковых моделей**: Повышение эффективности LLM
- **Сжатие нейронных сетей**: Сокращение размера моделей без потери качества
- **Эффективное машинное обучение**: Повышение производительности за счёт отбора важных признаков
- **Рекомендательные системы**: Оптимизация слоёв встраивания признаков
- **Компьютерное зрение**: Эффективная обработка изображений через отбор важных признаков

## Комплексное внимание (Complex Attention)

### Описание
Комплексное внимание - это подход к реализации механизма внимания, при котором используются комплексные числа для представления и обработки векторов запросов (queries), ключей (keys) и значений (values). В отличие от традиционного внимания, основанного на вещественных числах, комплексное внимание позволяет использовать как действительную, так и мнимую части комплексных чисел для кодирования дополнительной информации.

### Технические детали
- **Разделение каналов информации**: Действительная и мнимая части комплексных чисел могут кодировать разные аспекты взаимосвязей
- **RoPE++ реализация**: Использует обе части комплексного произведения - действительную для "локального" внимания и мнимую для "глобального" внимания
- **PoPE реализация**: Использует полярную форму комплексных чисел, разделяя магнитуду ("что") и фазу ("где")

### Преимущества
- **Более богатое представление**: Возможность кодировать дополнительную информацию через мнимую часть
- **Улучшенная экстраполяция**: Лучшая работа с длинными последовательностями за счет дополнительных степеней свободы
- **Явное разделение информации**: Возможность раздельного кодирования семантики и позиции

### Компромиссы
- **Удвоенное потребление памяти**: Хранение комплексных чисел требует в 2 раза больше памяти
- **Инженерная сложность**: Необходима специализированная реализация для эффективной работы с комплексными числами
- **Нужда в реобучении**: Как правило, модели с комплексным вниманием требуют обучения с нуля
...
- [[architectures/gated_attention_mechanism.md]] - Подробное описание Gated Attention, архитектурного решения для проблем стабильности, Attention Sink и Massive Activations
- [[architectures/attention_sink_phenomenon.md]] - Анализ феномена Attention Sink, который устраняется с помощью Gated Attention
- [[architectures/massive_activations_problem.md]] - Анализ проблемы Massive Activations, также решаемой с помощью Gated Attention
...
- [[models/hsa_ultralong.md]] - Подробное описание Hierarchical Sparse Attention (HSA) в контексте HSA-UltraLong архитектуры, модели с 16M контекстом
