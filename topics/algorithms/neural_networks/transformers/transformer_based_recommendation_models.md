...
- Усовершенствованная версия HSTU с инженерными оптимизациями
- **Ключевые инновации**:
  - Отказ от interleaved представления истории (↓32% FLOPs обучение, ↓63% инференс)
  - Load-Balanced Stochastic Length (+15% пропускной способности)
  - Semi-Local Attention с линейной сложностью (↑2.7× training efficiency, ↑5.1× inference efficiency)
  - Mixed precision BF16/FP8 (+10% обучение, +40% инференс)
  - Attention Truncation (↓1.5× FLOPs инференс)
- **Продакшен параметры**: 18 STU слоёв, история 16к, ~512 GPU
- **Результаты**: +4% consumption, +2-8% engagement

См. [[../../applications/recommendation_systems/research_papers/2024_2025_articles/ultra_hstu.md]] для подробного описания.

### OneTrans

- Архитектура, объединяющая моделирование последовательностей и взаимодействие признаков в едином трансформерном блоке
- Использует пирамидальную структуру для сжатия последовательных данных до сопоставимой длины с не-последовательными признаками
- Применяет казуальный трансформер с RMSNorm, Mixed Causal Attention и Mixed FFN
- Позволяет одновременно обрабатывать как последовательные, так и не-последовательные входы в рамках единой модели
...
Современные трансформерные модели для рекомендаций включают несколько важных архитектурных улучшений для решения проблем масштабирования:

- **LONGER**: использует Token Merging для уменьшения длины последовательности и сокращения вычислительной сложности аттеншна
- **OneTrans**: объединяет моделирование последовательностей и взаимодействие признаков в едином трансформерном блоке
- **DenseRec**: объединяет контентные и коллаборативные эмбеддинги для решения проблемы холодного старта

См. также: [[longer_architecture.md]], [[denserec.md]]
...
- [[unified_embeddings.md]] - Унифицированные эмбеддинги: подход к решению проблемы масштаба эмбеддинг-таблиц в трансформерных моделях
...
- [[mpformer.md]] - MPFormer: современный трансформерный фреймворк для мультизадачной персонализованной последовательной выборки
- [[cikm_25.md]] - CIKM'25: последние продвижения в трансформерных моделях рекомендаций
- [[das_dual_aligned_semantic_ids.md]] - DAS: новая модель, расширяющая применение трансформеров с интеграцией семантических и коллаборативных сигналов