...
YouTube DNN - одна из первых и наиболее влиятельных архитектур двух-башенной модели, представленная в знаменитой статье YouTube. Архитектура включает:

- **User Tower**: Обрабатывает историю пользователя, включая просмотренные видео, поисковые запросы и демографические признаки
- **Item Tower**: Обрабатывает признаки айтемов (видео), включая тайтлы, теги, категории и эмбеддинги контента
- **Обучение**: Использует задачу многоклассовой классификации, где модель предсказывает следующий просмотренный айтем из всего каталога
- **Масштабирование**: Использует sampled softmax для эффективного обучения на миллионах айтемов
- **Интерфейс**: На этапе инференса используется косинусное сходство между эмбеддингами для поиска релевантных айтемов

Архитектура YouTube DNN легла в основу многих современных рекомендательных систем и стала отправной точкой для разработки более сложных моделей.

См. также: [[youtubednn_architecture.md]] для более детального описания архитектуры.
...
- [[das_dual_aligned_semantic_ids.md]] - DAS: новая модель с интеграцией коллаборативного сигнала в обучение семантических ID для промышленных рекомендательных систем
- [[mpformer.md]] - MPFormer: адаптивный фреймворк для мультизадачной персонализованной последовательной выборки кандидатов
- [[cikm_25.md]] - CIKM'25: последние продвижения в генерации кандидатов, представленные на конференции
- [[embedding_stabilization/orthogonal_low_rank_embedding_stabilization.md]] - Ортогональная стабилизация низкоранговых эмбеддингов: метод стабилизации эмбеддингов в двухбашенных архитектурах