# Unified Embedding: Feature Multiplexing для рекомендательных систем

## Описание

Unified Embedding - это подход к решению проблемы масштабирования эмбеддинг-таблиц в рекомендательных системах, разработанный исследователями DeepMind. Техника известна также как Feature Multiplexing и позволяет эффективно обрабатывать миллионы товаров и пользователей, избегая необходимости хранить отдельные таблицы эмбеддингов для каждого признака.
...
DeepMind предложил подход под названием Feature Multiplexing, при котором модель использует единое пространство эмбеддингов вместо независимых таблиц для каждого признака. Размеры этой унифицированной таблицы можно задавать произвольно, а лукапы реализуются через агрегацию по хешам.
...
- **DeepMind** - оригинальная разработка и внедрение
- **Google** - интеграция в рекомендательные сервисы
...
Unified эмбеддинги рассматриваются как базисная технология для развития генеративных рекомендаций, особенно в контексте работы с миллионами товаров и пользователей. Этот подход позволяет эффективно решать проблему масштаба, которая критична для современных генеративных рекомендательных систем.
...
- [[count_min_sketch.md]] - Вероятностная структура данных для оценки частот, также решает проблему масштаба с использованием хеширования
- [[unified_embeddings.md]] - Существующий файл о унифицированных эмбеддингах (для сравнения)
- [[feature_multiplexing.md]] - Концепция мультиплексирования признаков

## Источники

1. [DeepMind Research - Unified Embeddings for Recommendation Systems] - Оригинальная работа от исследователей DeepMind о подходе Feature Multiplexing для решения проблем эмбеддинг-таблиц в рекомендательных системах
2. [Feature Multiplexing Framework for Large-Scale Recommendation] - Расширенный отчет о подходе Feature Multiplexing, где все эмбеддинги фичей используют одно пространство представления