...
Исследование подходов к масштабному запоминанию с сотнями триллионов параметров для генеративных последовательных рекомендательных систем. Работа представляет архитектуру VISTA (Visual Interface for Sequential Transduction Architecture) с двухступенчатым фреймворком: офлайн сжатие пожизненной истории пользователя в компактные эмбеддинги и рантайм использование этих эмбеддингов. Вместо тяжелого трансформера, обрабатывающего миллионы событий в контексте, архитектура использует квазилинейное внимание (QLA) и реконструкционный лосс для эффективного масштабирования.

Ключевые аспекты:
- Фиксированная стоимость инференса независимо от длины истории пользователя
- Возможность обработки до 1M событий в пожизненной истории
- Эмпирический рост метрик: +0.5% C-task, +0.2% O1 task, +0.04% O2 task в A/B тестировании на Reels
- Использование виртуальных "seed"-токенов для сжатия истории

См. также: [[massive_memorization_with_hundreds_of_trillions_of_parameters.md]], [[transformer_based_models.md]]
...
- [[2025_research_compilation_part2.md]] - Подборка статей 2025 (часть 2): продолжение подборки статей по рекомендательным системам, охватывающее фундаментальные модели и мультимодальность
