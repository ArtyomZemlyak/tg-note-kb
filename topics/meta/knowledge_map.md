# Карта Знаний

## Описание

Этот файл показывает связи между различными категориями и подкатегориями в базе знаний, чтобы облегчить навигацию и понимание взаимосвязей.

## Междоменные связи

### Математика → Computer Science
- [[math/algebra]] → [[computer_science/cs_fundamentals]]
- [[math/calculus_analysis]] → [[computer_science/distributed_systems]]
- [[math/statistics_probability]] → [[data_science/general]]

### Computer Science → Программирование
- [[computer_science/cs_fundamentals]] → [[programming/languages]]
- [[computer_science/distributed_systems]] → [[programming/methods_and_practices]]

### Программирование → Искусственный интеллект
- [[programming/languages/python]] → [[ai/models_and_architectures]]
- [[programming/methods_and_practices]] → [[ai/research_and_methodology]]

### Искусственный интеллект → Наука о данных
- [[ai/machine_learning]] (Moved to [[algorithms/classical_ml]] and [[algorithms/neural_networks]]) → [[data_science/general]]
- [[ai/models_and_architectures]] → [[data_science/tools]]

### Искусственный интеллект → Приложения
- [[ai/systems_and_technologies/agents]] → [[applications/yandex/yandex_alice]]
- [[ai/domain_specific_applications]] → [[applications/yandex/yandex_market]]

## Сквозные темы

### Инструменты
- [[cross_cutting_themes/tools]]
- [[programming/tools]]
- [[ai/applications_and_tools/tools]]
- [[data_science/tools]]

### Визуализация
- [[cross_cutting_themes/visualization]]
- [[data_science/visualization]]
- [[math/geometry]]

### Безопасность
- [[cross_cutting_themes/security]]
- [[ai/systems_and_technologies/security]]
- [[computer_science/security]]
- [[programming/security]]

### Этика
- [[cross_cutting_themes/ethics]]
- [[ai/research_and_methodology/ethics]]

## Методологии и Основы

### Методологии
- [[methodologies/research_methods]]
- [[methodologies/software_engineering]]
- [[methodologies/data_science_methods]]

### Основы
- [[foundations/mathematical_foundations]]
- [[foundations/computer_science_foundations]]