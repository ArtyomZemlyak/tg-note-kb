...
### Современные подходы: ToolOrchestra
Недавняя работа NVIDIA под названием ToolOrchestra представляет собой продвинутый подход к оркестрации, при котором:
- Используется легковесная модель-оркестратор (8B параметров на базе Qwen3-8B) вместо фронтир-моделей
- Оркестратор обучен с помощью алгоритма GRPO для умного маршрутизирования между различными инструментами и моделями
- Вектор предпочтений P = [p_outcome, p_compute, p_latency, ...] позволяет балансировать между точностью, вычислительными затратами и другими критериями
- Альтернатива "God Model" синдрому, при котором одна фронтир-модель пытается решить всё сама, включая задачи, которые могли бы эффективнее решаться специализированными моделями
...
- [[toolorchestra_framework.md]] - Современная реализация оркестрации моделей с использованием легковесного оркестратора
