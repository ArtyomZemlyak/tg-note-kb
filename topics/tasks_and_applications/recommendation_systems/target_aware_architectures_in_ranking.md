...
OneTrans - архитектура от TikTok, объединяющая моделирование последовательностей и взаимодействие признаков в едином трансформерном блоке. Модель одновременно обрабатывает как последовательные, так и не-последовательные входы через пирамидальную структуру, позволяя более эффективно моделировать сложные зависимости в пользовательском поведении и признаках. Архитектура использует казуальный трансформер с RMSNorm, Mixed Causal Attention и Mixed FFN, показывая значительный рост метрик в промышленных условиях.
